...
export function deltasFromTrace(
  trace: TraceFrame,
  account: Hex,
): { deltas: AssetDelta[]; approvals: ApprovalDelta[] } {
  ...
  const walk = (f: TraceFrame): void => {
    ...
    if (
      ...
      f.value !== '0x0' &&
      f.value !== '0x' &&
      (f.type === undefined || f.type === 'CALL' || f.type === 'CREATE' || f.type === 'CREATE2') &&
      !f.error
    ) {
      const v = BigInt(f.value)
      if (v > 0n) {
        if (f.from && same(f.from, account))
          deltas.push({
            asset: 'native',
            standard: 'native',
            amount: -v,
            ...(f.to ? { counterparty: f.to } : {}),
          })
        if (f.to && same(f.to, account))
          deltas.push({
            asset: 'native',
            standard: 'native',
            amount: v,
            ...(f.from ? { counterparty: f.from } : {}),
          })
      }
    }
    for (const log of f.logs ?? []) {
      ...
      if (t0 === TOPICS.transfer && log.topics.length === 3) {
        ...
        const [amount] = safeDecode(UINT, log.data, [0n])
        if (owner && spender && same(owner, account))
          approvals.push({ token: log.address, spender, amount, standard: 'erc20' })
        ...
        if (owner && operator && same(owner, account) && approved)
          approvals.push({
            token: log.address,
            spender: operator,
            amount: 'all',
...
function safeDecode<T extends readonly unknown[]>(
...
export function estimateSimulation(
  ...
  return revertReason === null
    ...
    : { mode: 'estimate', ok: false, revertReason, deltas: [], approvals: [], ...why }
}
