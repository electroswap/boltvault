...
export function TabBar({ items, activeId, onSelect, testID }: TabBarProps) {
  ...
  return (
    <Row
      backgroundColor="rgba(9, 13, 38, 0.9)"
      height={metrics.tabBar + insets.bottom}
      paddingBottom={insets.bottom}
      testID={testID}
      onLayout={(e: LayoutChangeEvent) => setWidth(e.nativeEvent.layout.width)}
    >
      <View
        style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 1, opacity: 0.55 }}
        pointerEvents="none"
      >
        <CurrentFill />
      </View>
      ...
      {width > 0 ? (
        <Animated.View
          ...
          style={{
            ...
            transform: [{ translateX: cell * index + cell / 2 - 9 }],
            transitionProperty: 'transform',
            transitionDuration: reduced ? 0 : motion.screen,
            transitionTimingFunction: cubicBezier(0.2, 0.8, 0.2, 1),
          }}
        >
