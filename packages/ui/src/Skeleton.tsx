  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        opacity,
        animationName: {
          from: { transform: [{ translateX: -band }] },
...
export function Skeleton({
  ...
  return (
    ...
    >
      {w > 0 ? (
        <Sweep width={w} opacity={0.16} durationMs={1800} reducedMotion={reducedMotion} />
      ) : null}
    </View>
  )
}
    ...
    <Column gap="$2" testID={testID}>
      {Array.from({ length: rows }, (_, i) => (
        <Row key={i} gap="$3" minHeight={metrics.busBar} paddingHorizontal="$3">
          {avatar ? (
            <Skeleton width={28} height={28} radius={14} reducedMotion={reducedMotion} />
          ) : null}
          <Column flex={1} gap={6}>
