...
import { metrics, motion, paint } from './tokens'
      ...
      {options.map((o) => {
        ...
        return (
          <Pressable key={o.id} onPress={() => onChange(o.id)} accessibilityRole="tab" accessibilityState={{ selected: active }} style={{ flex: 1, minHeight: metrics.hit, justifyContent: 'center', ...(compact ? { marginVertical: -7 } : {}) }}>
            <View style={{ flex: compact ? undefined : 1, height: compact ? 30 : undefined, justifyContent: 'center', alignItems: 'center', borderRadius: compact ? 8 : 11, overflow: 'hidden' }}>
              <Animated.View pointerEvents="none" style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, borderRadius: compact ? 8 : 11, borderWidth: 1, backgroundColor: paint.arcSoft, borderColor: paint.arcEdge, opacity: active ? 1 : 0, transitionProperty: 'opacity', transitionDuration: reduced ? 0 : motion.micro, transitionTimingFunction: 'ease-out' }} />
