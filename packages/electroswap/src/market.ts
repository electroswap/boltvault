...
const num = (a: z.infer<typeof amount>): number | null =>
  a && typeof a.value === 'number' && Number.isFinite(a.value) ? a.value : null
...
function tokenView(n: z.infer<typeof TokenNodeSchema>): MarketTokenView | null {
  ...
  return {
    ...
    safety:
      s === 'VERIFIED' || s === 'MEDIUM_WARNING' || s === 'STRONG_WARNING' || s === 'BLOCKED'
        ? s
        : null,
...
export async function fetchTopTokens(
  ...
  chainId: number,
): Promise<MarketTokenView[]> {
  const data = await client.query<unknown>(TOP_TOKENS, { chain: chainEnum(chainId) })
  const parsed = z
    .object({ topTokens: z.array(TokenNodeSchema.nullable()).nullable().optional() })
    .parse(data)
  const rows = (parsed.topTokens ?? [])
    .map((n) => (n ? tokenView(n) : null))
    .filter((x): x is MarketTokenView => x !== null)
...
export async function fetchTokenDetail(
  ...
  const parsed = z
    .object({
      token: TokenNodeSchema.extend({
        sparkline: z
          .object({
            priceHistory: z
              .array(z.object({ timestamp: z.number(), value: z.number() }).nullable())
              .nullable()
              .optional(),
          })
          .nullable()
  ...
  if (!base) return null
  const points = (n.sparkline?.priceHistory ?? [])
    .filter((p): p is { timestamp: number; value: number } => p !== null)
    .map((p) => ({ t: p.timestamp, v: p.value }))
  points.sort((a, b) => a.t - b.t)
  return {
    ...base,
    description: n.project?.description ?? null,
    homepageUrl: n.project?.homepageUrl ?? null,
    twitterUrl: n.project?.twitterUrl ?? null,
    telegramUrl: n.project?.telegramUrl ?? null,
    sparkline: points,
  }
}
...
export async function fetchPriceHistory(
  ...
  const data = await client.query<unknown>(PRICE_HISTORY, {
    address,
    chain: chainEnum(chainId),
    duration,
  })
  const parsed = z
    .object({
  ...
  if (!m) return null
  const points = (m.priceHistory ?? [])
    .filter(
      (p): p is { timestamp: number; value: number } => p !== null && Number.isFinite(p.value),
    )
    .map((p) => ({ t: p.timestamp, v: p.value }))
  points.sort((a, b) => a.t - b.t)
...
const CollectionNodeSchema = z.object({
  ...
  window: z
    .array(
      z.object({
        volume: amount,
        volumePercentChange: amount,
        floorPricePercentChange: amount,
        sales: amount,
      }),
    )
    .nullable()
...
function collectionView(n: z.infer<typeof CollectionNodeSchema>): CollectionView {
  ...
  const fee =
    (n.listingFees ?? []).find(
      (f): f is z.infer<typeof FeeSchema> => f !== null && f.basisPoints > 0,
    ) ?? null
  ...
  return {
    ...
    traits: (n.traits ?? [])
      .map((t) => ({ name: t.name ?? '', values: t.values ?? [] }))
      .filter((t) => t.name),
  }
}

const edges = <T extends z.ZodTypeAny>(node: T) =>
  z
    .object({
      edges: z.array(z.object({ node })).nullable().optional(),
      pageInfo: z
        .object({
...
export async function fetchTopCollections(
  ...
  const data = await client.query<unknown>(TOP_COLLECTIONS, {
    chains: [chainEnum(chainId)],
    first,
    listed,
    duration: window,
  })
...
export async function fetchCollections(
  client: ElectroSwapClient,
  chainId: number,
  filter: { addresses?: string[]; nameQuery?: string },
  first = 50,
): Promise<CollectionView[]> {
  const data = await client.query<unknown>(NFT_COLLECTIONS, {
    chain: chainEnum(chainId),
    filter,
    first,
  })
...
function orderView(n: z.infer<typeof OrderNodeSchema>): OrderView {
  ...
  const s = n.status
  const params =
    typeof n.protocolParameters === 'string'
      ? safeJson(n.protocolParameters)
      : (n.protocolParameters ?? null)
  return {
    type: t === 'LISTING' || t === 'OFFER' || t === 'BID' ? t : 'LISTING',
    status:
      s === 'VALID' || s === 'EXECUTED' || s === 'CANCELLED' || s === 'EXPIRED' || s === 'INVALID'
        ? s
        : 'INVALID',
...
const AssetNodeSchema = z.object({
  ...
  rarities: z
    .array(
      z.object({ rank: z.number().nullable().optional(), score: z.number().nullable().optional() }),
    )
    .nullable()
    .optional(),
  traits: z
...
function assetView(n: z.infer<typeof AssetNodeSchema>): AssetView {
  const listing =
    (n.listings?.edges ?? []).map((e) => orderView(e.node)).find((o) => o.status === 'VALID') ??
    null
  const bids = (n.bids?.edges ?? [])
    .map((e) => orderView(e.node))
    .filter((o) => o.status === 'VALID')
  bids.sort((a, b) => (b.priceEtn ?? 0) - (a.priceEtn ?? 0))
  const fee =
    (n.collection?.listingFees ?? []).find(
      (f): f is z.infer<typeof FeeSchema> => f !== null && f.basisPoints > 0,
    ) ?? null
  ...
  return {
    ...
    traits: (n.traits ?? [])
      .map((t) => ({ name: t.name ?? '', value: t.value ?? '', rarity: t.rarity ?? null }))
      .filter((t) => t.name),
...
export async function fetchAssets(
  ...
  const data = await client.query<unknown>(NFT_ASSETS, {
    chain: chainEnum(chainId),
    address,
    orderBy: opts.orderBy ?? 'PRICE',
    asc: opts.asc ?? true,
    filter,
    first: opts.first ?? 40,
  ...
  return {
    assets: (page?.edges ?? []).map((e) => assetView(e.node)),
...
export async function fetchAsset(
  ...
  const data = await client.query<unknown>(NFT_ASSET_DETAILS, {
    chain: chainEnum(chainId),
    address,
    tokenId,
  })
...
export async function fetchOwnedAssets(
  ...
  owner: string,
  first = 100,
): Promise<OwnedAssetView[]> {
  const out: OwnedAssetView[] = []
  let after: string | null = null
  for (let page = 0; page < 5; page++) {
    const data = await client.query<unknown>(NFT_BALANCES, {
      chain: chainEnum(chainId),
      owner,
      first,
      after,
    ...
    const parsed = z
      .object({
        nftBalances: edges(
          z.object({
            quantity: z.number().nullable().optional(),
            listedMarketplaces: z.array(z.string()).nullable().optional(),
            ownedAsset: AssetNodeSchema.nullable().optional(),
          }),
        ),
      })
      .parse(data)
    ...
    for (const e of p?.edges ?? []) {
      ...
      out.push({
        ...
        listed: (e.node.listedMarketplaces ?? []).length > 0 || a.listing !== null,
      })
    }
...
export async function fetchCollectionBalances(
  client: ElectroSwapClient,
  chainId: number,
  owner: string,
): Promise<CollectionBalanceView[]> {
  const data = await client.query<unknown>(NFT_COLLECTION_BALANCES, {
    chain: chainEnum(chainId),
    address: owner,
  })
  const parsed = z
    .object({
      nftCollectionBalances: z
        .object({
          collections: z
            .array(
              z
    ...
    .parse(data)
  return (parsed.nftCollectionBalances?.collections ?? [])
    .filter((c): c is NonNullable<typeof c> => c !== null)
    .map((c) => ({
      address: c.address,
      name: c.name,
      logoUrl: c.logoImage ?? null,
      balance: c.balance,
    }))
}
...
export async function fetchNftActivity(
  client: ElectroSwapClient,
  chainId: number,
  filter: { address?: string; tokenId?: string },
  first = 40,
): Promise<NftActivityView[]> {
  const data = await client.query<unknown>(NFT_ACTIVITY, {
    chain: chainEnum(chainId),
    filter,
    first,
  })
...
export async function fetchBids(
  ...
  const data = await client.query<unknown>(NFT_BIDS, {
    chain: chainEnum(chainId),
    address: bidder,
    statuses: ['VALID'],
    first,
        ...
        z.object({
          ...
          asset: z
            .object({
              ...
              smallImage: url,
              ownerAddress: z.string().nullable().optional(),
              nftContract: z.object({ address: z.string() }).nullable().optional(),
              collection: z
                .object({
                  name: z.string().nullable().optional(),
                  listingFees: z.array(FeeSchema.nullable()).nullable().optional(),
  ...
  return (parsed.nftBids?.edges ?? []).flatMap((e) => {
    ...
    const fee =
      (n.asset?.collection?.listingFees ?? []).find(
        (f): f is z.infer<typeof FeeSchema> => f !== null && f.basisPoints > 0,
      ) ?? null
    const order = orderView({
      ...
      endAt: n.expiresAt,
      maker: bidder,
      protocolParameters: n.protocolParameters,
    })
    return [
      {
        order,
        address,
        tokenId,
        name: n.asset?.name ?? `#${tokenId}`,
        imageUrl: str(n.asset?.smallImage),
        owner: n.asset?.ownerAddress ?? null,
        collectionName: n.asset?.collection?.name ?? n.collection?.name ?? 'Collection',
        creatorFee: fee ? { payoutAddress: fee.payoutAddress, basisPoints: fee.basisPoints } : null,
...
export async function fetchBidObligation(
  client: ElectroSwapClient,
  chainId: number,
  bidder: string,
): Promise<bigint> {
  const data = await client.query<unknown>(NFT_BID_OBLIGATION, {
    chain: chainEnum(chainId),
    address: bidder,
  })
  const parsed = z
    .object({
      nftBidObligation: z
        .object({ wetnObligation: z.string().nullable().optional() })
        .nullable()
        .optional(),
    })
    .parse(data)
...
const PresaleNodeSchema = z.object({
  ...
  token: z.object({
    ...
    totalSupply: z.number().nullable().optional(),
    address: z.string().nullable().optional(),
    liquidityPool: z.string().nullable().optional(),
  }),
  campaign: z.object({
    manager: z.string(),
    creator: z.string(),
    ...
    contributors: z
      .array(
        z.object({
          ...
          amountEtn: z.number().nullable().optional(),
...
export interface CampaignView {
  ...
  readonly token: {
    ...
    readonly totalSupply: number | null
    readonly liquidityPool: string | null
  }
...
function campaignView(n: z.infer<typeof PresaleNodeSchema>): CampaignView {
  ...
  return {
    pool: n.pool,
    status:
      s === 'ACTIVE' || s === 'LAUNCHED' || s === 'FAILED' || s === 'CANCELLED' || s === 'PENDING'
        ? s
        : 'PENDING',
    ...
    token: {
      ...
      address: n.token.address ?? null,
      totalSupply: n.token.totalSupply ?? null,
      liquidityPool: n.token.liquidityPool ?? null,
    },
    creator: c.creator,
...
export async function fetchCampaigns(
  ...
  const data = await client.query<unknown>(PRESALES, {
    chain: chainEnum(chainId),
    filter,
    first: 50,
  })
...
export async function fetchCampaign(
  ...
  const data = await client.query<unknown>(PRESALE, {
    pool,
...
const FarmNodeSchema = z.object({
  ...
  farmer: z
    .object({
      ...
      boltMultiplier: z.number(),
      boltDeposited: z.string(),
      startingBlock: z.number(),
      rewards: z.string(),
      thirdPartyRewards: z.string(),
      farmOwnership: z.number(),
    })
    .nullable()
    .optional(),
})
...
export interface FarmIndexView {
  ...
  readonly thirdParty: {
    ...
    readonly symbol: string
    readonly tokensPerBlock: string
  ...
  readonly farmer: {
    ...
    readonly boltDeposited: string
    readonly startingBlock: number
    readonly rewards: string
    readonly thirdPartyRewards: string
...
export async function fetchFarms(
  ...
  farmer?: string,
  active?: boolean,
): Promise<FarmIndexView[]> {
  const data = await client.query<unknown>(YIELD_FARMS, {
    chain: chainEnum(chainId),
    farmer: farmer ?? null,
    active: active ?? null,
  })
  ...
  return parsed.yieldFarms.map((n) => ({
    ...
    thirdParty: n.thirdPartyReward
      ? {
          token: n.thirdPartyReward.token,
          symbol: n.thirdPartyReward.symbol,
          tokensPerBlock: n.thirdPartyReward.tokensPerBlock,
          endBlock: n.thirdPartyReward.endBlock,
        }
      : null,
    farmer: n.farmer
      ? {
          liquidity: n.farmer.liquidity,
          durationMultiplier: n.farmer.durationMultiplier,
          boltMultiplier: n.farmer.boltMultiplier,
          boltDeposited: n.farmer.boltDeposited,
          startingBlock: n.farmer.startingBlock,
          rewards: n.farmer.rewards,
          thirdPartyRewards: n.farmer.thirdPartyRewards,
          ownership: n.farmer.farmOwnership,
        }
      : null,
  }))
}
