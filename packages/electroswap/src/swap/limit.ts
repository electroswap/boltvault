...
export const ORDERS_ABI = [{ type: 'function', name: 'orders', stateMutability: 'view', inputs: [{ name: '', type: 'uint256' }], outputs: ORDER_OUTPUTS }] as const
...
export function encodeSubmitOrder(input: { tokenIn: Hex; tokenOut: Hex; unwrapOutput: boolean; amountInExact: bigint; amountOutMin: bigint; recipient: Hex; durationSeconds: bigint }): Hex {
  return encodeFunctionData({ abi: LIMIT_ORDERS_ABI, functionName: 'submitOrder', args: [input.tokenIn, input.tokenOut, input.unwrapOutput, input.amountInExact, input.amountOutMin, input.recipient, input.durationSeconds] })
}

export function encodeSubmitOrderWithPermit(input: { tokenIn: Hex; tokenOut: Hex; unwrapOutput: boolean; amountInExact: bigint; amountOutMin: bigint; recipient: Hex; durationSeconds: bigint; permit: { token: Hex; amount: bigint; expiration: number; nonce: number; spender: Hex; sigDeadline: bigint; signature: Hex } }): Hex {
  const p = input.permit
  return encodeFunctionData({
    abi: LIMIT_ORDERS_ABI,
    functionName: 'submitOrderWithPermit',
    args: [input.tokenIn, input.tokenOut, input.unwrapOutput, input.amountInExact, input.amountOutMin, input.recipient, input.durationSeconds, { details: { token: p.token, amount: p.amount, expiration: p.expiration, nonce: p.nonce }, spender: p.spender, sigDeadline: p.sigDeadline }, p.signature],
  })
}
...
export function encodeCloseOrders(orderIds: readonly bigint[]): Hex {
  return encodeFunctionData({ abi: LIMIT_ORDERS_ABI, functionName: 'closeOrders', args: [[...orderIds]] })
}

/** The user's open orders, read in one batch. */
export async function openOrders(manager: Hex, user: Hex, read: Reader, nowSeconds: number): Promise<LimitOrderView[]> {
  const [ids] = await read([{ address: manager, abi: LIMIT_ORDERS_ABI, functionName: 'openOrdersByUser', args: [user] }])
  if (!ids?.ok || !Array.isArray(ids.value)) return []
  const orderIds = ids.value as readonly bigint[]
  if (orderIds.length === 0) return []
  const calls: ReadCall[] = orderIds.map((id) => ({ address: manager, abi: ORDERS_ABI, functionName: 'orders', args: [id] }))
  const rows = await read(calls)
  const out: LimitOrderView[] = []
  rows.forEach((r) => {
    if (!r.ok || !Array.isArray(r.value)) return
    const v = r.value as unknown as readonly [bigint, Hex, Hex, Hex, Hex, boolean, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, number]
