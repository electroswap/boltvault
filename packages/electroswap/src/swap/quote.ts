...
export function candidates(
  ...
  for (const fee of V3_FEES)
    out.push({
      kind: 'v3',
      label: `V3 ${fee / 10_000}%`,
      route: { hops: [{ kind: 'v3', tokenIn, tokenOut, fee }] },
    })
  ...
  */
  ...
  const perBase: Candidate[][] = bases.map((base) => {
    const forBase: Candidate[] = [
      {
        kind: 'v2',
        label: 'V2 via',
        route: {
          hops: [
            { kind: 'v2', tokenIn, tokenOut: base },
            { kind: 'v2', tokenIn: base, tokenOut },
          ],
        },
      },
    ]
    if (withMixed) {
      forBase.push({
        ...
        route: {
          hops: [
            { kind: 'v2', tokenIn, tokenOut: base },
            { kind: 'v3', tokenIn: base, tokenOut, fee: 3000 },
          ],
        },
      })
      forBase.push({
        ...
        route: {
          hops: [
            { kind: 'v3', tokenIn, tokenOut: base, fee: 3000 },
            { kind: 'v2', tokenIn: base, tokenOut },
          ],
        },
      })
    ...
    for (const f1 of HOP_FEES) {
      for (const f2 of HOP_FEES) {
        forBase.push({
          kind: 'v3',
          label: `V3 ${f1 / 10_000}% → ${f2 / 10_000}%`,
          route: {
            hops: [
              { kind: 'v3', tokenIn, tokenOut: base, fee: f1 },
              { kind: 'v3', tokenIn: base, tokenOut, fee: f2 },
            ],
          },
        })
      }
...
function callFor(c: Candidate, amountIn: bigint, a: QuoteAddresses): ReadCall {
  ...
  if (c.kind === 'mixed') {
    return {
      address: a.mixedRouteQuoter as Hex,
      abi: MIXED_ROUTE_QUOTER_ABI,
      functionName: 'quoteExactInput',
      args: [v3PackedPath(c.route.hops), amountIn],
    }
  }
...
function parse(c: Candidate, r: ReadResult): RouteQuote | null {
  ...
  if (c.kind === 'v2') {
    ...
    return last !== undefined && last > 0n
      ? { candidate: c, amountOut: last, gasEstimate: 120_000n * BigInt(c.route.hops.length) }
      : null
  }
  ...
  return typeof amountOut === 'bigint' && amountOut > 0n
    ? { candidate: c, amountOut, gasEstimate: typeof gas === 'bigint' ? gas : 150_000n }
    : null
}
...
export async function bestRoute(
  ...
  const quotes = cands
    .map((c, i) => parse(c, results[i] ?? { ok: false }))
    .filter((q): q is RouteQuote => q !== null)
  if (quotes.length === 0) return null
  quotes.sort((a, b) =>
    a.amountOut === b.amountOut
      ? a.candidate.route.hops.length - b.candidate.route.hops.length
      : b.amountOut > a.amountOut
        ? 1
        : -1,
  )
  const top = quotes[0] as RouteQuote
  const single = quotes.find(
    (q) => q.candidate.route.hops.length === 1 && q.amountOut * 1000n >= top.amountOut * 999n,
  )
...
function callForExactOut(c: Candidate, amountOut: bigint, a: QuoteAddresses): ReadCall {
  ...
  return {
    ...
    args: [v3PackedPathExactOut(c.route.hops), amountOut],
  }
}
...
function parseExactOut(c: Candidate, r: ReadResult): RouteQuoteExactOut | null {
  ...
  if (c.kind === 'v2') {
    ...
    return typeof first === 'bigint' && first > 0n
      ? { candidate: c, amountIn: first, gasEstimate: 120_000n * BigInt(c.route.hops.length) }
      : null
  }
  const tuple = r.value as readonly [bigint, unknown, unknown, bigint]
  const amountIn = tuple[0]
  const gas = tuple[3]
  return typeof amountIn === 'bigint' && amountIn > 0n
    ? { candidate: c, amountIn, gasEstimate: typeof gas === 'bigint' ? gas : 150_000n }
    : null
}
 ...
 */
export async function bestRouteExactOut(
  tokenIn: Hex,
  tokenOut: Hex,
  amountOut: bigint,
  addresses: QuoteAddresses,
  read: Reader,
): Promise<BestQuoteExactOut | null> {
  ...
  const quotes = cands
    .map((c, i) => parseExactOut(c, results[i] ?? { ok: false }))
    .filter((q): q is RouteQuoteExactOut => q !== null)
  if (quotes.length === 0) return null
  quotes.sort((a, b) =>
  ...
  const single = quotes.find(
    (q) => q.candidate.route.hops.length === 1 && q.amountIn * 1000n <= top.amountIn * 1001n,
  )
  return { best: single ?? top, all: quotes }
}
