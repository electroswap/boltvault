...
export const WARP_ROUTES: readonly WarpRoute[] = [
  {
    ...
    endpoints: [
      {
        chainId: 52014,
        router: '0x3187deAd7A2Bd6770F5Fe81495D1B715926AAe6e',
      ...
      {
        chainId: 1,
        router: '0xFC2944e9F1d57Ce82aeD05922887DD660404e50B',
      ...
      {
        chainId: 8453,
        router: '0xaaDF9558Cf103d394B22b18Ffbaa0D1c0778Ccfa',
  ...
  },
  {
    ...
    endpoints: [
      {
        chainId: 52014,
        router: '0x48E722f1458b253c2FB0E573F939318D7Dbd54e7',
        token: '0x48E722f1458b253c2FB0E573F939318D7Dbd54e7',
        standard: 'synthetic',
        symbol: 'USDT',
        decimals: 6,
      },
      {
        chainId: 1,
        router: '0x97B80b1d89d10E5d2c9396B0ea0BA2dAf917Dc96',
...
export function warpEndpoint(chainId: number, token: string): WarpEndpoint | null {
  for (const route of WARP_ROUTES)
    for (const e of route.endpoints) if (e.chainId === chainId && same(e.token, token)) return e
  return null
}
...
export function warpAsset(
  ...
  return {
    symbol: e.symbol,
    destinations: corridorsFrom(chainId, token).map((c) => c.destination.chainId),
  }
}
...
export function encodeTransferRemote(
  ...
  return encodeFunctionData({
    ...
    args: [destinationDomain, recipientBytes32(recipient), amount],
  })
}
...
export function dispatchIdFrom(
 ...
 */
export function verificationCalls(
  c: Corridor,
): Array<{
  address: Hex
  abi: typeof TOKEN_ROUTER_ABI
  functionName: 'mailbox' | 'domains' | 'wrappedToken'
  args: readonly []
}> {
  const calls: Array<{
    address: Hex
  ...
  if (c.origin.standard === 'collateral')
    calls.push({
...
export function verifyCorridor(
  c: Corridor,
  results: ReadonlyArray<{ ok: boolean; value?: unknown }>,
  ...
  if (!mailbox?.ok || typeof mailbox.value !== 'string' || !same(mailbox.value, chain.mailbox))
    return { ok: false, reason: 'router mailbox mismatch' }
  ...
  if (
    ...
    !(domains.value as readonly number[]).some((d) => Number(d) === dest.domain)
  )
    return { ok: false, reason: 'destination domain not enrolled' }
  if (
    c.origin.standard === 'collateral' &&
    (!wrapped?.ok || typeof wrapped.value !== 'string' || !same(wrapped.value, c.origin.token))
  )
    return { ok: false, reason: 'collateral router wraps a different token' }
