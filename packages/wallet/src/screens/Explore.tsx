...
export function Explore({
  ...
  const tokens = useCached<ExploreToken[]>({
    key:
      segment === 'tokens' || segment === 'collectibles'
        ? cacheKey('explore', 'tokens', ETN)
        : null,
  ...
  const collections = useCached<CollectionView[]>({
    key:
      segment === 'collectibles'
        ? cacheKey('explore', 'collections', ETN, accountId ?? '-', window)
        : null,
    cached: (e) =>
      e.explore.cachedCollections({ chainId: ETN, ...(accountId ? { accountId } : {}), window }),
    fresh: (e) =>
      e.explore.collections({ chainId: ETN, ...(accountId ? { accountId } : {}), window }),
    maxAgeMs: 60_000,
  })
  ...
  useEffect(() => {
    ...
    const id = setTimeout(
      () =>
        engine.explore.search({ chainId: ETN, query }).then(
          (r) => alive && setFound(r),
          () => undefined,
        ),
      250,
  ...
  const pin = (address: string, pinned: boolean): void => {
    ...
    setFound((f) =>
      f
        ? {
            ...f,
            tokens: f.tokens.map((x) => (x.address === address ? { ...x, pinned: !pinned } : x)),
          }
        : f,
    )
  }
  ...
  const etnUsd =
    (tokens.value ?? []).find((x) => x.address === 'native' || x.symbol === 'ETN')?.price ?? null
  return (
    <Column flex={1}>
      <ScrollView contentContainerStyle={{ padding: inset, gap: 14 }} testID="explore">
        <PageHeader
          title={
            segment === 'tokens'
              ? t({ id: 'explore.tokens', message: 'Tokens' })
              : segment === 'collectibles'
                ? t({ id: 'explore.collections.title', message: 'Collections' })
                : segment === 'launch'
                  ? t({ id: 'explore.launchpad', message: 'Launchpad' })
                  : t({ id: 'explore.farms', message: 'Farms' })
          }
          right={
            <>
              {segment === 'collectibles' ? (
                <IconButton
                  ...
                  onPress={() => setAddCollectionOpen(true)}
                  testID="explore-add-collection"
                />
              ) : null}
              {segment === 'tokens' && host.browser ? (
                <IconButton
                  icon="external"
                  label={t({ id: 'explore.browser', message: 'Browser' })}
                  onPress={() => router.navigate('browser')}
                  testID="explore-browser"
                />
              ) : null}
              {segment === 'tokens' || segment === 'launch' ? (
                <IconButton
                  icon="bell"
                  label={t({ id: 'explore.alerts', message: 'Alerts' })}
                  badge={unread}
                  onPress={() => router.navigate('alerts')}
                  testID="explore-alerts"
                />
              ) : null}
            </>
          }
        ...
        {segment === 'tokens' ? (
          <Input
            value={query}
            onChange={setQuery}
            placeholder={t({ id: 'explore.search', message: 'Search tokens and collections' })}
            autoFocus={search}
        ...
        {available === false ? (
          <Plate gap="$2" testID="explore-unavailable">
            <Body size="title">
              {t({ id: 'explore.off.title', message: 'Markets are on Electroneum' })}
            </Body>
            <Body tone="mute" size="caption">
              {t({
                id: 'explore.off.body',
                message:
                  'Prices, collections and campaigns come from ElectroSwap, which this build cannot reach yet. Your balances, sends and swaps still work.',
              })}
            </Body>
          </Plate>
        ) : null}
        {found ? (
          <Column gap="$2" testID="explore-results">
            {found.tokens.map((x) => (
              <TokenRow
                key={x.address}
                token={x}
                onPress={() => router.navigate('token', { chainId: ETN, address: x.address })}
                onPin={() => pin(x.address, x.pinned)}
              />
            ))}
            {found.collections.map((c) => (
              <CollectionCard
                key={c.address}
                collection={c}
                onPress={() => router.navigate('collection', { chainId: ETN, address: c.address })}
                onStar={() => void star('collection', c.address, c.name, c.starred)}
              />
            ))}
          ...
          <>
            <FreshnessLine
              ...
              testID="explore-freshness"
            />
            {current.freshness === 'loading' ? null : segment === 'tokens' ? (
              ...
              <Column gap="$3" testID="explore-collections">
                ...
                <Row gap="$2" alignItems="center" justifyContent="space-between">
                  {active ? (
                    <Pill
                      label={t({ id: 'explore.rack', message: 'Your collection' })}
                      icon={<Icon name="nft" size={14} color={paint.arc} />}
                      size="sm"
                ...
                <Column testID="collections-list">
                  {(collections.value ?? []).map((c, i, arr) => (
                    <CollectionRankRow
                      ...
                      etnUsd={etnUsd}
                      onPress={() =>
                        router.navigate('collection', { chainId: ETN, address: c.address })
                      }
                      last={i === arr.length - 1}
                    />
                  ))}
              ...
              <Column gap="$2" testID="sky">
                {(campaigns.value ?? []).map((c) => (
                  <CampaignCard
                    ...
                    onStar={() => void star('campaign', c.pool, c.token.symbol, c.starred)}
                  />
                ))}
                {(campaigns.value ?? []).length === 0 ? (
                  <Body tone="mute" size="caption">
                    {t({
                      id: 'sky.empty',
                      message:
                        'No campaigns right now. Star one from a share link to be told when it goes live.',
                    })}
                  </Body>
                ) : null}
              </Column>
      ...
      <AddTokenSheet
        ...
        initialAddress={query.trim()}
        reducedMotion={reducedMotion}
      />
      <AddCollectionSheet
        open={addCollectionOpen}
        onClose={() => setAddCollectionOpen(false)}
        onAdded={() => collections.refresh()}
        reducedMotion={reducedMotion}
      />
    </Column>
  )
    ...
    <SharedElement id={tokenSharedId(token.chainId, token.address)}>
      <Row gap="$2" alignItems="center" minHeight={52} testID={`explore-token-${token.symbol}`}>
        ...
        >
          ...
          <Column flex={1}>
            <Row gap="$2" alignItems="center">
              ...
              {token.safety === 'BLOCKED' || token.safety === 'STRONG_WARNING' ? (
            ...
            <Body tone="mute" size="caption" numberOfLines={1}>
              {token.volume24h !== null
                ? t({
                    id: 'explore.vol',
                    message: 'Vol {v}',
                    values: { v: formatFiat(token.volume24h, 'USD') },
                  })
                : token.name}
            </Body>
          </Column>
        ...
        <IconButton
          ...
          active={token.pinned}
          onPress={onPin}
          testID={`star-token-${token.symbol}`}
        />
      </Row>
    </SharedElement>
  )
