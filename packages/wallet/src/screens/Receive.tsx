...
export function Receive({
  ...
  useEffect(() => {
    ...
    engine.settings.get().then(
      (s) => setEnabled([ETN, ...s.enabledChains]),
      () => undefined,
    )
  ...
  if (requesting && amount.trim()) {
    try {
      if (token === 'native')
        uri = `ethereum:${address}@${chainId}?value=${parseUnits(amount, 18).toString()}`
      else if (selected)
        uri = `ethereum:${selected.address}@${chainId}/transfer?address=${address}&uint256=${parseUnits(amount, selected.decimals).toString()}`
    } catch {
  ...
  */
  ...
  const shareText = t({
    id: 'receive.share.text',
    message: '{label} on {chain} (chain {id}): {address}',
    values: { label: active.label, chain: chainName, id: chainId, address },
  })
  const share = async (): Promise<void> => {
    const title = t({
      id: 'receive.share.title',
      message: 'My {chain} address',
      values: { chain: chainName },
    })
    if (host.share) {
  ...
  */
      ...
      setVerify(
        r.address.toLowerCase() === address.toLowerCase()
          ...
          : {
              state: 'problem',
              message: t({
                id: 'receive.verify.differs',
                message: 'The device answered with a different address. Do not use this one.',
              }),
            },
      )
    } catch (err) {
  ...
  return (
    <Column flex={1}>
      <ScrollView
        contentContainerStyle={{ padding: inset, gap: 14, alignItems: 'stretch' }}
        testID="receive"
      >
        <PageHeader
          title={t({ id: 'receive.title', message: 'Receive' })}
          subtitle={t({
            id: 'receive.subtitle',
            message: 'Only send {c} assets to this address',
            values: { c: chain?.name ?? 'Electroneum' },
          })}
        />
        <Row testID="receive-chain">
          <ChainSelectPill
            chainId={chainId}
            label={chain?.name ?? 'Electroneum'}
            onPress={() => setChainOpen(true)}
            testID="receive-chain-select"
          />
        </Row>
        ...
        <Plate role="raised" gap="$3" alignItems="center" testID="receive-plate">
          ...
          <Row gap="$2" flexWrap="wrap" justifyContent="center">
            <Key
              label={
                copied
                  ...
                  : t({ id: 'receive.copy', message: 'Copy address' })
              }
              size="compact"
              onPress={copy}
              testID="receive-copy"
            ...
            <Key
              label={
                shared
                  ? t({ id: 'receive.share.copied', message: 'Link copied' })
                  : t({ id: 'receive.share', message: 'Share' })
              }
              kind="secondary"
              size="compact"
        ...
        {deviceName ? (
          <Plate gap="$2" testID="receive-verify">
            <Body size="title">
              {t({
                id: 'receive.verify.title',
                message: 'Check it on your {d}',
                values: { d: deviceName },
              })}
            </Body>
            <Body tone="mute" size="caption">
              {t({
                id: 'receive.verify.body',
                message:
                  'Malware can change an address on its way to the screen. Your {d} holds the keys, so what it shows is the address that can actually spend what arrives — compare the two, character for character.',
                values: { d: deviceName },
              })}
            </Body>
            {active.kind === 'keystone' ? (
              <Body tone="mute" size="caption" testID="receive-verify-keystone">
                {t({
                  id: 'receive.verify.keystone',
                  message:
                    'A Keystone has no cable to ask over. Open the account on the device itself and compare the address it shows with the one above.',
                })}
              </Body>
            ) : (
              <>
                <Key
                  label={
                    verify?.state === 'asking'
                      ? t({ id: 'receive.verify.asking', message: 'Look at the device…' })
                      : t({ id: 'receive.verify.key', message: 'Show the address on the device' })
                  }
                  kind="secondary"
                  size="compact"
                ...
                {verify?.state === 'match' ? (
                  <Row gap="$2" alignItems="center" testID="receive-verify-match">
                    ...
                    <Body tone="arc" size="caption">
                      {t({
                        id: 'receive.verify.match',
                        message: 'The device shows this address. It is safe to receive here.',
                      })}
                    </Body>
                  </Row>
                ) : null}
        ...
        <Plate gap="$2" testID="receive-request">
          <Row justifyContent="space-between">
            ...
            <Body
              tone="arc"
              size="caption"
              onPress={() => setRequesting((v) => !v)}
              testID="receive-request-toggle"
            >
              ...
              <Row gap="$2" flexWrap="wrap">
                {tokens.slice(0, 6).map((x) => (
                  <Pill
                    key={x.address}
                    label={x.symbol}
                    selected={x.address === token}
                    size="sm"
                    onPress={() => setToken(x.address)}
                  />
                ))}
              </Row>
              <Input
                ...
                placeholder="0"
                hint={t({
                  id: 'receive.request.hint',
                  message: 'The code above updates with the amount.',
                })}
        ...
        <Body tone="mute" size="caption">
          {t({
            id: 'receive.warning',
            message:
              'This is Electroneum Smart Chain (52014). Send ETN here from an exchange that supports the smart chain. Do not send from the old Electroneum app.',
          })}
        </Body>
      </ScrollView>
      <ChainSheet
        ...
        options={enabled.map((id) => ({
          id,
          name: chains.find((c) => c.chainId === id)?.name ?? `Chain ${id}`,
          ...(id === ETN
