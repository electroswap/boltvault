...
import {
  ...
  RenameSheet,
  type MenuItem,
} from '../components/accounts/AccountSheets'
...
type Group = {
  id: string
  title: string
  icon: 'key' | 'lock' | 'hardware' | 'eye'
  items: AccountView[]
  seed?: SeedView
}
type SheetState =
  | { kind: 'menu' | 'details' | 'rename' | 'confirm'; account: AccountView }
  | { kind: 'seedMenu' | 'renameSeed' | 'reveal'; seed: SeedView }
  | { kind: 'add' }
  | null
...
export function Accounts({ body }: { body: 'extension-popup' | 'extension-tab' | 'mobile' }) {
  ...
  const [ledger, setLedger] = useState<{
    available: boolean
    devices: ReadonlyArray<{ deviceId: string; model: string }>
  } | null>(null)
  ...
  useEffect(() => {
    ...
    let alive = true
    engine.hardware.ledgerStatus().then(
      (s) => alive && setLedger(s),
      () => alive && setLedger(null),
    )
    return () => {
      alive = false
    }
  ...
  const connectLedger = (): void => {
    if (host.requestHid)
      void host
        .requestHid()
        .then(() => engine.hardware.ledgerStatus())
  ...
  const matches = (a: AccountView): boolean =>
    !q || a.label.toLowerCase().includes(q) || a.address.toLowerCase().includes(q)
  ...
  for (const s of seeds)
    groups.push({
      ...
      title: s.label,
      icon: 'key',
      items: accounts.filter((a) => a.seedId === s.id),
      seed: s,
    })
  ...
  if (imported.length)
    groups.push({
      id: 'imported',
      title: t({ id: 'acct.imported', message: 'Imported keys' }),
      icon: 'lock',
      items: imported,
    })
  const hardware = others.filter(
    (a) => a.kind === 'ledger' || a.kind === 'trezor' || a.kind === 'keystone',
  )
  ...
  for (const [key, items] of devices) {
    ...
    groups.push({
      id: `hw-${key}`,
      title: kind === 'ledger' ? 'Ledger' : kind === 'trezor' ? 'Trezor' : 'Keystone',
      icon: 'hardware',
      items,
    })
  }
  ...
  if (watching.length)
    groups.push({
      id: 'watch',
      title: t({ id: 'acct.watch', message: 'Watching' }),
      icon: 'eye',
      items: watching,
    })
      ...
      [
        {
          id: 'rename',
          icon: 'edit',
          label: t({ id: 'acct.rename', message: 'Rename' }),
          onPress: () => setSheet({ kind: 'rename', account: a }),
          testID: 'menu-rename',
        },
        {
          id: 'details',
          icon: 'info',
          label: t({ id: 'acct.details', message: 'Details' }),
          onPress: () => setSheet({ kind: 'details', account: a }),
          testID: 'menu-details',
        },
        {
          id: 'up',
          icon: 'chevronUp',
          label: t({ id: 'acct.moveUp', message: 'Move up' }),
          onPress: () => move(a, -1),
          testID: 'menu-up',
        },
        {
          id: 'down',
          icon: 'chevronDown',
          label: t({ id: 'acct.moveDown', message: 'Move down' }),
          onPress: () => move(a, 1),
          testID: 'menu-down',
        },
        {
          ...
          label: a.hidden
            ? t({ id: 'acct.show', message: 'Show' })
            : t({ id: 'acct.hide', message: 'Hide' }),
          onPress: () => {
          ...
          ? [
              {
                id: 'remove',
                icon: 'trash' as const,
                label: t({ id: 'acct.remove', message: 'Remove' }),
                tone: 'burn' as const,
                onPress: () => setSheet({ kind: 'confirm', account: a }),
                testID: 'menu-remove',
              },
            ]
          : []),
      ] as MenuItem[]
    ).filter((item) => !(isActive && (item.id === 'hide' || item.id === 'remove')))
  const seedMenu = (s: SeedView): MenuItem[] => [
    {
      id: 'next',
      icon: 'plus',
      label: t({ id: 'acct.derive', message: 'Add the next address' }),
      onPress: () => {
        void run(() => engine.accounts.derive({ seedId: s.id }))
        setSheet(null)
      },
      testID: 'seed-derive',
    },
    {
      id: 'rename',
      icon: 'edit',
      label: t({ id: 'acct.renameSeed', message: 'Rename this wallet' }),
      onPress: () => setSheet({ kind: 'renameSeed', seed: s }),
      testID: 'seed-rename',
    },
    ...(!s.backedUp
      ? [
          {
            ...
            label: t({ id: 'acct.backup', message: 'Back up now' }),
            onPress: () => {
              setSheet(null)
      ...
      ? [
          {
            id: 'reveal',
            icon: 'eye' as const,
            label: t({ id: 'acct.reveal', message: 'Reveal recovery phrase' }),
            onPress: () => setSheet({ kind: 'reveal', seed: s }),
            testID: 'seed-reveal',
          },
        ]
      : []),
  ...
  return (
    <Column flex={1}>
      <ScrollView contentContainerStyle={{ padding: inset, gap: 12 }} testID="accounts">
        <PageHeader
          title={t({ id: 'accounts.title', message: 'Accounts' })}
          right={
            <Pill
              label={t({ id: 'acct.add', message: 'Add' })}
              icon={<Icon name="plus" size={14} color={paint.arc} />}
              tone="arc"
              onPress={() => setSheet({ kind: 'add' })}
              testID="add-account-open"
            />
          }
        />
        {/*
        ...
        */}
          ...
          <Plate role="raised" gap="$2" testID="current-hardware">
            <Row gap="$2" alignItems="center" justifyContent="space-between" minHeight={36}>
              <Row gap={6} alignItems="center" flexShrink={1}>
                <Dot
                  color={ledger && ledger.devices.length > 0 ? paint.surge : paint.mute}
                  size={6}
                />
                <Body tone="mute" size="caption" numberOfLines={1}>
                  {ledger && ledger.devices.length > 0
                    ? t({ id: 'acct.ledger.on', message: 'Ledger connected' })
                    : t({ id: 'acct.ledger.off', message: 'Ledger is not connected' })}
                </Body>
              </Row>
              {!(ledger && ledger.devices.length > 0) ? (
                <Pill
                  label={t({ id: 'acct.ledger.connect', message: 'Connect' })}
                  tone="arc"
                  size="sm"
                  onPress={connectLedger}
                  testID="current-connect"
                />
              ) : null}
            </Row>
          </Plate>
        ...
        {accounts.length > 6 ? (
          <Input
            value={query}
            onChange={setQuery}
            placeholder={t({ id: 'acct.search', message: 'Search accounts' })}
          ...
          return (
            ...
            >
              <Row alignItems="center" gap="$2" minHeight={36}>
                ...
                {hidden.length ? (
                  <Pressable
                    onPress={() => setShowHidden((s) => ({ ...s, [g.id]: !open }))}
                    accessibilityRole="button"
                    accessibilityLabel={t({ id: 'acct.hidden.toggle', message: 'Hidden accounts' })}
                    style={{
                      minHeight: 44,
                      marginVertical: -4,
                      justifyContent: 'center',
                  ...
                  >
                    <Row gap={4} alignItems="center">
                      <Body tone="mute" size="caption">
                        {hidden.length === 1
                          ? t({ id: 'acct.hidden.one', message: '1 hidden' })
                          : t({
                              id: 'acct.hidden.many',
                              message: '{n} hidden',
                              values: { n: hidden.length },
                            })}
                      </Body>
                      <Icon
                ...
                {g.seed ? (
                  g.seed.backedUp ? (
                    ...
                    <Pill
                      label={t({ id: 'acct.backup.short', message: 'Back up' })}
                      tone="ember"
                      size="sm"
                      onPress={() => router.navigate('backup')}
                      testID={`backup-${g.seed.id}`}
                    />
                  )
                ) : null}
                {g.seed ? (
                  <IconButton
                    icon="more"
                    label={t({ id: 'acct.seedMenu', message: 'Wallet options' })}
                    onPress={() => setSheet({ kind: 'seedMenu', seed: g.seed as SeedView })}
                    testID={`seed-menu-${g.seed.id}`}
                  />
                ) : null}
              </Row>
              {visible.map((a) => (
                <AccountRow
                  ...
                  onSelect={() => select(a)}
                  onMenu={() => setSheet({ kind: 'menu', account: a })}
                  onCopy={host.copy ? () => copy(a) : undefined}
          ...
          <Plate gap="$2" testID="accounts-empty">
            <Body tone="mute">
              {t({
                id: 'acct.empty',
                message:
                  'No accounts yet. Add a recovery phrase, a key, an address to watch or a hardware wallet.',
              })}
            </Body>
            <Key
              label={t({ id: 'acct.add', message: 'Add' })}
      ...
      <MenuSheet
        ...
        title={sheet?.kind === 'menu' ? sheet.account.label : ''}
        items={
          sheet?.kind === 'menu' ? menuFor(sheet.account, sheet.account.id === active?.id) : []
        }
        reducedMotion={reducedMotion}
        testID="account-menu"
      />
      <MenuSheet
        open={sheet?.kind === 'seedMenu'}
        onClose={() => setSheet(null)}
        title={sheet?.kind === 'seedMenu' ? sheet.seed.label : ''}
        items={sheet?.kind === 'seedMenu' ? seedMenu(sheet.seed) : []}
        reducedMotion={reducedMotion}
        testID="seed-menu"
      />
      <AccountDetailsSheet
        ...
        onClose={() => setSheet(null)}
        account={sheet?.kind === 'details' ? sheet.account : null}
        seed={
          sheet?.kind === 'details'
            ? (seeds.find((s) => s.id === sheet.account.seedId) ?? null)
            : null
        }
        reducedMotion={reducedMotion}
      />
      <RenameSheet
        ...
        onClose={() => setSheet(null)}
        title={t({ id: 'acct.rename', message: 'Rename' })}
        value={sheet?.kind === 'rename' ? sheet.account.label : ''}
        onSave={async (label) => {
          if (sheet?.kind === 'rename')
            await run(() => engine.accounts.rename({ id: sheet.account.id, label }))
        }}
        reducedMotion={reducedMotion}
        testID="rename-account"
      />
      <RenameSheet
        open={sheet?.kind === 'renameSeed'}
        onClose={() => setSheet(null)}
        title={t({ id: 'acct.renameSeed', message: 'Rename this wallet' })}
        value={sheet?.kind === 'renameSeed' ? sheet.seed.label : ''}
        onSave={async (label) => {
          if (sheet?.kind === 'renameSeed')
            await run(() => engine.accounts.renameSeed({ seedId: sheet.seed.id, label }))
        }}
        reducedMotion={reducedMotion}
        testID="rename-seed"
      />
      <ConfirmSheet
        ...
        title={t({
          ...
          values: { label: sheet?.kind === 'confirm' ? sheet.account.label : '' },
        })}
        body={
          sheet?.kind === 'confirm' && sheet.account.kind === 'imported'
            ? t({
                id: 'acct.remove.key',
                message:
                  'This key is not part of a recovery phrase. Without its own backup, anything at this address is gone for good.',
              })
            : t({
                id: 'acct.remove.body',
                message:
                  'BoltVault stops showing this address. Nothing on the chain changes; you can add it again any time.',
              })
        }
        confirmLabel={t({ id: 'acct.remove', message: 'Remove' })}
        onConfirm={() => {
          if (sheet?.kind === 'confirm')
            void run(() => engine.accounts.remove({ id: sheet.account.id }))
          setSheet(null)
        }}
        reducedMotion={reducedMotion}
      ...
      />
      <SeedRevealSheet
        open={sheet?.kind === 'reveal'}
        onClose={() => setSheet(null)}
        seed={sheet?.kind === 'reveal' ? sheet.seed : null}
        reducedMotion={reducedMotion}
      />
      <AddAccountSheet
        open={sheet?.kind === 'add'}
...
function SeedRevealSheet({
  ...
  const show = async (
    read: () => Promise<{ mnemonic: string } | null>,
    failure?: string,
  ): Promise<void> => {
    setBusy(true)
    setError(null)
  ...
  return (
    <Sheet
      ...
      title={t({
        id: 'reveal.title',
        message: 'Recovery phrase · {label}',
        values: { label: seed.label },
      })}
      quiet
      reducedMotion={reducedMotion}
      footer={
        words ? (
          <Key
            label={t({ id: 'reveal.hide', message: 'Hide' })}
            kind="secondary"
            size="compact"
            onPress={onClose}
          />
          ...
          <Key
            label={t({ id: 'reveal.key', message: 'Reveal' })}
            size="compact"
      ...
      {words ? (
        ...
        <Column gap="$2">
          <Body tone="mute">
            {t({
              id: 'reveal.body',
              message: 'Enter your password. Make sure nobody can see your screen.',
            })}
          </Body>
          <Input
            value={password}
          ...
          {passkeyOk ? (
            <Key
              label={t({ id: 'reveal.passkey', message: 'Reveal with passkey' })}
              kind="secondary"
              size="compact"
              disabled={busy}
              onPress={() => void revealWithPasskey()}
              testID="reveal-passkey"
            />
          ) : null}
          {biometricOk ? (
            <Key
              label={t({ id: 'reveal.biometric', message: 'Reveal with biometrics' })}
              kind="secondary"
              size="compact"
              disabled={busy}
              onPress={() => void revealWithBiometric()}
              testID="reveal-biometric"
            />
          ) : null}
          {error ? <Body tone="burn">{error}</Body> : null}
        </Column>
