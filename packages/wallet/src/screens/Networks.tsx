...
export function Networks({ body }: { body: 'extension-popup' | 'extension-tab' | 'mobile' }) {
  ...
  const set = (patch: Partial<Settings>): void => {
    ...
    const p =
      patch.showTestnet === false && settings
        ? { ...patch, enabledChains: settings.enabledChains.filter((c) => c !== ETN_TESTNET) }
        : patch
    ...
    try {
      ...
      setProblem(
        err instanceof Error
          ? err.message
          : t({ id: 'networks.rpc.bad', message: 'That RPC did not answer with this chain id.' }),
      )
    } finally {
      setBusy(false)
    }
  ...
  const shown = chains.filter(
    (c) => !c.testnet || settings?.showTestnet || c.chainId === ETN_TESTNET,
  )
    ...
    <ScrollView contentContainerStyle={{ padding: inset, gap: 14 }} testID="networks">
      ...
      <Body tone="mute" size="caption">
        {t({
          id: 'networks.body',
          message:
            'Electroneum is home and always on. Other chains show balances, send, receive, activity and approvals through public RPCs; swaps and markets stay on Electroneum.',
        })}
      </Body>

      <Plate gap="$2" testID="networks-testnet">
        <Toggle
          value={settings?.showTestnet ?? false}
          onChange={(v) => set({ showTestnet: v })}
          label={t({ id: 'networks.testnet', message: 'Show Electroneum testnet' })}
          hint={t({
            id: 'networks.testnet.hint',
            message: 'Chain 5201420. Test ETN has no value; names do not resolve there.',
          })}
          testID="networks-testnet-toggle"
        />
      </Plate>
        ...
        .map((c) => {
          ...
          return (
            <Plate
              key={c.chainId}
              gap="$2"
              role={home ? 'raised' : undefined}
              testID={`network-${c.chainId}`}
            >
              <Row justifyContent="space-between" alignItems="center">
                ...
                {home ? null : (
                  <Toggle
                    value={on}
                    onChange={(v) => toggle(c.chainId, v)}
                    label=""
                    testID={`network-${c.chainId}-toggle`}
                  />
                )}
              </Row>
              {isEditing ? (
                <Column gap="$2">
                  <Input
                    ...
                    error={problem}
                    autoFocus
                    testID={`network-${c.chainId}-rpc-input`}
                  />
                  <Row gap="$2">
                    <Key
                      label={t({ id: 'networks.rpc.save', message: 'Use this RPC' })}
                      disabled={busy || !/^https:\/\//i.test(url.trim())}
                      onPress={() => void saveRpc(c.chainId, url.trim())}
                      testID={`network-${c.chainId}-rpc-save`}
                    />
                    {custom ? (
                      <Key
                        label={t({ id: 'networks.rpc.clear', message: 'Back to default' })}
                        kind="secondary"
                        disabled={busy}
                        onPress={() => void saveRpc(c.chainId, null)}
                        testID={`network-${c.chainId}-rpc-clear`}
                      />
                    ) : null}
                    <Key
                      label={t({ id: 'cancel', message: 'Cancel' })}
                      kind="secondary"
                      onPress={() => (setEditing(null), setProblem(null))}
                    />
                  </Row>
                </Column>
              ) : (
                <Row gap="$2">
                  <Key
                    label={
                      custom
                        ? t({ id: 'networks.rpc.edit', message: 'Change RPC' })
                        : t({ id: 'networks.rpc.add', message: 'Custom RPC' })
                    }
                    kind="secondary"
                    onPress={() => (setEditing(c.chainId), setUrl(custom ?? ''), setProblem(null))}
                    testID={`network-${c.chainId}-rpc`}
                  />
                  {c.explorerUrl ? (
                    <Body
                      tone="mute"
