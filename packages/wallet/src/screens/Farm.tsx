...
export function Farm({
  ...
  useEffect(() => {
    let alive = true
    engine.farm.farm({ chainId, farmId, ...(active ? { accountId: active.id } : {}) }).then(
      (f) => alive && setFarm(f),
            ...
            if (farm) {
              if (lastEdited === 0)
                setAmount1(
                  q.amount1Raw === '0'
                    ? ''
                    : formatRaw(q.amount1Raw, farm.decimals1).replace(/,/g, ''),
                )
              else
                setAmount0(
                  q.amount0Raw === '0'
                    ? ''
                    : formatRaw(q.amount0Raw, farm.decimals0).replace(/,/g, ''),
                )
            }
          },
          () => alive && setQuote(null),
  ...
  useEffect(() => {
    ...
    let alive = true
    engine.farm.quoteWithdraw({ accountId: active.id, chainId, farmId, percent, asNative }).then(
      (q) => alive && setWq(q),
      () => alive && setWq(null),
    )
    return () => {
  ...
  if (flow) {
    ...
    return (
      <FlowPlate
        ...
        titles={{
          working: t({ id: 'farm.working', message: 'Working…' }),
          done:
            last === 'collect'
              ? t({ id: 'farm.collected', message: 'Collected' })
              : last === 'withdraw'
                ? t({ id: 'farm.withdrawn', message: 'Withdrawn' })
                : t({ id: 'farm.deposited', message: 'Deposited' }),
        }}
        summary={farm?.name ?? null}
        onDone={dismiss}
        testID="farm-flow"
      />
    )
  ...
  const hasNative = farm
    ? [farm.symbol0, farm.symbol1].includes('ETN') || [farm.symbol0, farm.symbol1].includes('WETN')
    : false
    ...
    <Column flex={1}>
      <ScrollView contentContainerStyle={{ padding: inset, gap: 12 }} testID="farm">
        <PageHeader
          ...
          title={farm ? undefined : t({ id: 'farm.title', message: 'Farm' })}
          right={
            farm ? (
              <>
                {!farm.active ? (
                  <Pill
                    label={t({ id: 'farm.closed', message: 'Closed' })}
                    tone="ember"
                    size="sm"
                  />
                ) : null}
        ...
        {farm ? (
          <>
            <Row justifyContent="center">
              ...
              */}
              <Coil
                durationMultiplier={p?.durationMultiplier ?? 10_000}
                boltMultiplier={p?.boltMultiplier ?? 10_000}
                glow={glow}
                size={body === 'extension-popup' ? 190 : 240}
                at2x={dateLabel(p?.at2x ?? null)}
                at25x={dateLabel(p?.at25x ?? null)}
                reducedMotion={reducedMotion}
                testID="coil"
              />
            </Row>
            {!farm.active ? (
              <Plate gap={2} testID="farm-closed">
                <Body tone="ember" size="caption">
                  {p
                    ? t({
                        id: 'farm.closed.position',
                        message:
                          'This farm no longer takes deposits. Your position still earns what is left; you can collect and withdraw.',
                      })
                    : t({ id: 'farm.closed.body', message: 'This farm no longer takes deposits.' })}
                </Body>
              </Plate>
            ) : null}
            {p ? (
                ...
                <Row justifyContent="space-between" alignItems="center">
                  ...
                  <Pill
                    label={t({
                      id: 'farm.share',
                      message: '{p}% of farm',
                      values: { p: (p.shareOfFarm * 100).toFixed(2) },
                    })}
                    size="xs"
                  />
                </Row>
                ...
                */}
                <StatStrip
                  ...
                  cells={[
                    ...
                    {
                      label: t({ id: 'farm.stat.collect', message: 'To collect' }),
                      value: `${formatAmount(p.pendingRewards, 18)} DYNO`,
                      tone: 'surge' as const,
                      ...(farm.thirdParty && BigInt(p.pendingThirdParty) > 0n
                        ? {
                            caption: `+${formatAmount(p.pendingThirdParty, 18)} ${farm.thirdParty.symbol}`,
                          }
                        : {}),
                    },
                  ]}
                ...
                <Column gap={2}>
                  <Body tone="mute" size="caption">
                    {BigInt(p.boltDeposited) > 0n
                      ? t({
                          id: 'farm.boost',
                          message: '{b} BOLT boosting at {m}×',
                          values: {
                            b: formatAmount(p.boltDeposited, 18),
                            m: (p.boltMultiplier / 10_000).toFixed(2),
                          },
                        })
                      : t({ id: 'farm.boost.none', message: 'No BOLT boost yet' })}
                    {p.nextStair
                      ? ` · ${t({ id: 'farm.nextStair', message: '{b} more BOLT for {m}×', values: { b: formatAmount(p.nextStair.more, 18), m: (p.nextStair.multiplier / 10_000).toFixed(2) } })}`
                      : ''}
                  </Body>
              ...
              <Plate gap="$1" testID="farm-none">
                <Body tone="mute" size="caption">
                  {t({
                    id: 'farm.none',
                    message:
                      'Deposit both sides of the pair to start earning DYNO. Your multiplier grows with time and with BOLT deposited as a boost.',
                  })}
                </Body>
              </Plate>
            ) : null}
            ...
            {active ? (
              <Row gap="$2" testID="farm-keys">
                {farm.active ? (
                  <Column flex={1}>
                    <Key
                      label={t({ id: 'farm.deposit', message: 'Deposit' })}
                      disabled={busy}
                ...
                {p ? (
                  <Column flex={1}>
                    <Key
                      label={t({ id: 'farm.withdraw', message: 'Withdraw' })}
                      kind="secondary"
                      disabled={busy}
                      onPress={() => setSheet('withdraw')}
                      testID="farm-withdraw"
                    />
                  </Column>
                ) : null}
                {p ? (
                  <Column flex={1}>
                    <Key
                      ...
                      kind="secondary"
                      disabled={busy || BigInt(p.pendingRewards) === 0n}
                      onPress={() =>
                        void run(() =>
                          engine.farm.collect({
                            accountId: active.id,
            ...
            */}
            <StatStrip
              ...
              cells={[
                ...
                },
                {
                  ...
                  value: farm.tvlUsd !== null ? formatFiat(farm.tvlUsd, 'USD') : '—',
                },
                {
                  label: t({ id: 'farm.stat.farmers', message: 'Farmers' }),
                  value: String(farm.farmerCount),
                },
              ]}
              testID="farm-stats"
            />
            {p && BigInt(p.pendingRewards) > 0n && p.nextStair && farm.active ? (
              <Plate role="card" gap="$1" testID="farm-boost-plate">
                <Body fontWeight="600">
                  {t({ id: 'farm.cb.title', message: 'Collect & boost' })}
                </Body>
                <Body tone="mute" size="caption">
                  {t({
                    id: 'farm.cb.body',
                    message:
                      'Collect your DYNO, swap it for BOLT, then deposit BOLT as a boost with your next deposit. Boosts land on the 50,000 or 100,000 BOLT stairs — you need {b} more for {m}×.',
                    values: {
                      b: formatRaw(p.nextStair.more, 18),
                      m: (p.nextStair.multiplier / 10_000).toFixed(2),
                    },
                  })}
                </Body>
                <Key
                  label={t({ id: 'farm.cb.swap', message: 'Swap DYNO for BOLT' })}
                  kind="secondary"
                  size="compact"
                  onPress={() =>
                    router.setTab('swap', {
                      tokenIn: '0xEe432C220273e4F949007B4c1946562826Efa055',
                      tokenOut: '0x043fAa1b5C5FC9a7dc35171f290c29ECDE0cCff1',
                    })
                  }
                  testID="farm-cb-swap"
                />
      ...
      <Sheet
        ...
        title={t({ id: 'farm.deposit.title', message: 'Deposit' })}
        reducedMotion={reducedMotion}
        footer={
          <Key
            label={t({ id: 'farm.deposit', message: 'Deposit' })}
            ...
            testID="farm-deposit-go"
          />
        }
        testID="farm-deposit-sheet"
      >
        {farm ? (
          <Column gap="$3">
            <Input
            ...
            <Body tone="mute" size="caption">
              {t({
                id: 'farm.ratio',
                message: 'The pool sets the ratio; unused amounts come back to you.',
              })}
            </Body>
            <Input
              value={bolt}
              onChange={setBolt}
              placeholder="0"
              label={t({ id: 'farm.boltBoost', message: 'BOLT boost (optional)' })}
              testID="farm-bolt"
            />
                ...
                return (
                  <Pill
                    ...
                    label={t({
                      id: 'farm.stair',
                      message: '{b} → {m}×',
                      values: { b: more.toString(), m: s === '50000' ? '1.05' : '1.15' },
                    })}
                    size="sm"
                    onPress={() => setBolt(more.toString())}
                    testID={`farm-stair-${s}`}
                  />
                )
              })}
              ...
              <Plate gap={2} testID="farm-dilution">
                <Body tone="ember" size="caption">
                  {t({
                    id: 'farm.dilution',
                    message:
                      'A second deposit re-weights your duration bonus: {a}× today → {b}× after this deposit. It climbs again from there.',
                    values: {
                      a: (quote.multiplierBefore / 10_000).toFixed(2),
                      b: (quote.multiplierAfter / 10_000).toFixed(2),
                    },
                  })}
                </Body>
              </Plate>
            ...
            {quote?.boltStair && BigInt(quote.boltRaw) > 0n ? (
              <Body tone="arc" size="caption">
                {t({
                  id: 'farm.stair.land',
                  message: 'Lands on the {b} BOLT stair · {m}×',
                  values: {
                    b: formatRaw(quote.boltStair.total, 18),
                    m: (quote.boltStair.multiplier / 10_000).toFixed(2),
                  },
                })}
              </Body>
            ) : null}
            ...
            {quote?.ok ? (
              <Body tone="mute" size="caption">
                {t({
                  id: 'farm.steps',
                  message:
                    '{n} signature{s}: the farm is allowed to pull each token once, then the deposit.',
                  values: { n: quote.steps.length, s: quote.steps.length === 1 ? '' : 's' },
                })}
              </Body>
            ) : null}
            {error ? <Body tone="burn">{error}</Body> : null}
      ...
      <Sheet
        open={sheet === 'withdraw'}
        onClose={() => setSheet(null)}
        title={t({ id: 'farm.withdraw.title', message: 'Withdraw' })}
        reducedMotion={reducedMotion}
        footer={
          <Key
            label={t({ id: 'farm.withdraw', message: 'Withdraw' })}
            disabled={busy || !wq?.ok || percent === 0}
            onPress={() =>
              void run(() =>
                engine.farm.withdraw({
                  accountId: active?.id ?? '',
                  chainId,
                  farmId,
                  percent,
                  asNative: hasNative && asNative,
                }),
              )
            }
            testID="farm-withdraw-go"
          />
        }
        testID="farm-withdraw-sheet"
      >
        {farm && p ? (
          <Column gap="$3">
            ...
            {hasNative ? (
              <Toggle
                value={asNative}
                onChange={setAsNative}
                label={t({ id: 'farm.asNative', message: 'Receive ETN instead of WETN' })}
                testID="farm-as-native"
              />
            ) : null}
            {wq ? (
              <Plate gap={2} testID="farm-withdraw-preview">
                <Body size="caption">
                  {t({ id: 'farm.w.leaves', message: 'What leaves the farm' })}
                </Body>
                <Body tone="mute" size="caption">
                ...
                <Body tone="arc" size="caption">
                  {t({
                    id: 'farm.w.collected',
                    message: '{d} DYNO collected with it',
                    values: { d: formatRaw(wq.rewardsRaw, 18) },
                  })}
                </Body>
                {BigInt(wq.boltReturnedRaw) > 0n ? (
                  <Body tone="ember" size="caption">
                    {t({
                      id: 'farm.w.bolt',
                      message:
                        '{b} BOLT boost comes back — it only unlocks when you withdraw everything',
                      values: { b: formatRaw(wq.boltReturnedRaw, 18) },
                    })}
                  </Body>
                ) : (
                  <Body tone="mute" size="caption">
                    {wq.keepsMultiplier
                      ? t({
                          id: 'farm.w.keeps',
                          message:
                            'You keep your duration multiplier. BOLT unlocks only when you withdraw everything.',
                        })
                      : ''}
                  </Body>
                )}
