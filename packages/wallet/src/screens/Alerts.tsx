  ...
  */
  ...
  const openTarget = (target: string | null): void => {
    ...
    if (target.startsWith('farm:'))
      return router.navigate('farm', { chainId: ETN, farmId: Number(target.slice(5)) })
    if (target.startsWith('campaign:'))
      return router.navigate('campaign', { chainId: ETN, pool: target.slice(9) })
  }
  ...
  const save = async (i: WatchItem): Promise<void> => {
    ...
    const num = (s: string | undefined, fallback: number | null): number | null =>
      s === undefined
        ? fallback
        : s.trim() === ''
          ? null
          : Number.isFinite(Number(s))
            ? Number(s)
            : fallback
    await engine.watchlist.setAlert({
      kind: i.kind,
      chainId: i.chainId,
      address: i.address,
      above: num(d?.above, i.above),
      below: num(d?.below, i.below),
      onLive: i.onLive,
    })
  }
    ...
    <ScrollView contentContainerStyle={{ padding: inset, gap: 14 }} testID="alerts">
      ...
      <Body tone="mute" size="caption">
        {t({
          id: 'alerts.body',
          message:
            'Star a token, collection or campaign from Explore, then set what to tell you about. Rewards to collect and dividends to claim are mentioned once a day on their own. Nothing here nags.',
        })}
      </Body>
      {notes.length > 0 ? (
        <Column gap="$2" testID="alerts-inbox">
          <Row justifyContent="space-between" alignItems="center">
            ...
            <Key
              label={t({ id: 'alerts.clear', message: 'Clear' })}
              kind="secondary"
              size="compact"
              onPress={clearNotes}
              testID="alerts-clear"
        ...
        <Plate gap="$2" testID="alerts-push">
          <Row justifyContent="space-between" alignItems="center">
            ...
            <Key
              label={
                pushState === 'granted'
                  ? t({ id: 'alerts.push.off', message: 'Turn off' })
                  : t({ id: 'alerts.push.on', message: 'Turn on' })
              }
              kind="secondary"
              disabled={pushState === 'denied'}
              onPress={() =>
                void (pushState === 'granted'
                  ? host.push?.disable().then(() => refreshPush())
                  : host.push?.enable().then(() => refreshPush()))
              }
              testID="alerts-push-toggle"
            />
          </Row>
          <Body tone="mute" size="caption">
            {pushState === 'granted'
              ? t({
                  id: 'alerts.push.granted',
                  message:
                    'On. Incoming funds, sales and offers, campaigns going live, rewards and dividends arrive while the app is closed.',
                })
              : pushState === 'denied'
                ? t({
                    id: 'alerts.push.denied',
                    message: 'Notifications are off for BoltVault in the system settings.',
                  })
                : t({
                    id: 'alerts.push.off.body',
                    message:
                      'Off. Turn it on to hear about incoming funds, sales, campaigns and rewards while the app is closed. Only a type and an id ever travel; the app fetches the details.',
                  })}
          </Body>
        </Plate>
      ) : null}
      {items.length === 0 ? (
        <Plate gap="$2" testID="alerts-empty">
          ...
          <Key
            label={t({ id: 'alerts.explore', message: 'Explore' })}
            kind="secondary"
            onPress={() => router.navigate('explore')}
            testID="alerts-explore"
          />
        </Plate>
      ) : null}
      {items.map((i) => (
        <Plate key={key(i)} gap="$2" testID={`alert-${i.kind}-${i.address}`}>
          <Row justifyContent="space-between" alignItems="center">
            <Column>
              ...
              <Body tone="mute" size="caption">
                ...
                {i.lastValue !== null && i.kind !== 'campaign'
                  ? ` · ${i.kind === 'token' ? '$' : ''}${i.lastValue}${i.kind === 'collection' ? ' ETN' : ''}`
                  : ''}
              </Body>
            </Column>
            <Chip
              onPress={() =>
                void engine.watchlist.unstar({
                  kind: i.kind,
                  chainId: i.chainId,
                  address: i.address,
                })
          ...
          {i.kind === 'campaign' ? (
            <Toggle
              value={i.onLive}
              onChange={(v) =>
                void engine.watchlist.setAlert({
                  kind: i.kind,
                  chainId: i.chainId,
                  address: i.address,
                  above: i.above,
              ...
              label={t({ id: 'alerts.onLive', message: 'Tell me when it goes live' })}
              testID={`alert-live-${i.address}`}
            />
          ) : (
            <Row gap="$2" alignItems="flex-end">
              <Column flex={1}>
                <Input
                  value={drafts[key(i)]?.above ?? (i.above === null ? '' : String(i.above))}
                  onChange={(v) =>
                    setDrafts((d) => ({
                      ...d,
                      [key(i)]: {
                        above: v,
                        below: d[key(i)]?.below ?? (i.below === null ? '' : String(i.below)),
                      },
                    }))
                  }
                  label={
                    i.kind === 'token'
                      ? t({ id: 'alerts.above', message: 'Above $' })
                      : t({ id: 'alerts.aboveEtn', message: 'Floor above (ETN)' })
                  }
                  placeholder="—"
                  testID={`alert-above-${i.address}`}
                />
              </Column>
              <Column flex={1}>
                <Input
                  value={drafts[key(i)]?.below ?? (i.below === null ? '' : String(i.below))}
                  onChange={(v) =>
                    setDrafts((d) => ({
                      ...d,
                      [key(i)]: {
                        above: d[key(i)]?.above ?? (i.above === null ? '' : String(i.above)),
                        below: v,
                      },
                    }))
                  }
                  label={
                    i.kind === 'token'
                      ? t({ id: 'alerts.below', message: 'Below $' })
                      : t({ id: 'alerts.belowEtn', message: 'Floor below (ETN)' })
                  }
                  placeholder="—"
                  testID={`alert-below-${i.address}`}
                />
              </Column>
              <Key
      ...
      {items.length ? (
        <Row gap="$2" alignItems="center">
          <Key
            label={t({ id: 'alerts.check', message: 'Check now' })}
            kind="secondary"
            onPress={() => engine.watchlist.check().then(setChecked, () => setChecked([]))}
            testID="alerts-check"
          />
          {checked ? (
            <Body tone="mute" size="caption" testID="alerts-checked">
              {checked.length
                ? t({ id: 'alerts.sent', message: '{n} sent', values: { n: checked.length } })
                : t({ id: 'alerts.quiet', message: 'Nothing to tell you' })}
            </Body>
          ) : null}
        </Row>
      ) : null}
