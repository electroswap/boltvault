...
const LINKS: Array<{
  key: 'website' | 'twitter' | 'telegram' | 'discord'
  icon: IconName
  label: () => string
}> = [
...
export function Campaign({
  ...
  useEffect(() => {
    let alive = true
    engine.launchpad.detail({ chainId, pool, ...(active ? { accountId: active.id } : {}) }).then(
      (v) => alive && setC(v),
    ...
    const timer = setInterval(
      () =>
        engine.launchpad
          .detail({ chainId, pool, ...(active ? { accountId: active.id } : {}) })
          .then(
            (v) => alive && setC(v),
            () => undefined,
          ),
  ...
  if (flow) {
    ...
    return (
      <FlowPlate
        ...
        titles={{
          working: t({ id: 'campaign.working', message: 'Working…' }),
          done:
            last === 'contribute'
              ? t({ id: 'campaign.contributed', message: 'Contributed' })
              : t({ id: 'campaign.claimed', message: 'Claimed' }),
        }}
        summary={c?.token.name ?? null}
        onDone={dismiss}
        testID="campaign-flow"
      />
    )
  }
    ...
    switch (v.phase) {
      case 'live':
        return t({
          ...
          message: 'Until {d}',
          values: {
            d: new Date(v.ends * 1000).toLocaleString('en-GB', {
              day: 'numeric',
              month: 'short',
              hour: '2-digit',
              minute: '2-digit',
      ...
      case 'upcoming':
        return t({
          ...
          message: 'Starts {d}',
          values: {
            d: new Date(v.starts * 1000).toLocaleString('en-GB', {
              day: 'numeric',
              month: 'short',
              hour: '2-digit',
              minute: '2-digit',
      ...
      case 'failed':
        return t({
          id: 'campaign.failed',
          message: 'Did not reach its target — contributions are refundable',
        })
      case 'cancelled':
        return t({
          id: 'campaign.cancelled',
          message: 'Cancelled by the team — contributions are refundable',
        })
    }
  }
  const alertPill =
    c && alertable(c) ? (
      <Pill
        label={
          c.starred
            ? t({ id: 'campaign.starred', message: 'Alerts on' })
            : t({ id: 'campaign.alert', message: 'Alert me' })
        }
        icon={<Icon name="star" size={14} color={c.starred ? paint.ember : paint.mute} />}
        selected={c.starred}
        size="sm"
        onPress={() =>
          void engine.watchlist[c.starred ? 'unstar' : 'star']({
            kind: 'campaign',
            chainId,
            address: pool,
            label: c.token.symbol,
          }).then(() => setC({ ...c, starred: !c.starred }))
        }
        testID="campaign-star"
      />
    ) : null
  ...
  return (
    <Column flex={1}>
          ...
          <>
            ...
            <SharedElement id={campaignSharedId(chainId, pool)}>
              <Column testID="campaign-hero">
                <Column
                  ...
                  overflow="hidden"
                  position="relative"
                  backgroundColor="rgba(55, 166, 255, 0.10)"
                >
                  {c.bannerUrl ? (
                    <Artwork
                      uri={c.bannerUrl}
                      label={c.token.name}
                      size={{ width: 1000, height: 72 }}
                      radius={0}
                    />
                  ) : (
                    <Column
                    ...
                    <Row gap="$2" alignItems="center">
                      ...
                      <Body tone="mute" size="caption" numberOfLines={1}>
                        {t({
                          id: 'campaign.by',
                          message: 'by {a}',
                          values: { a: c.creatorName ?? shortAddress(c.creator) },
                        })}
                      </Body>
                    </Row>
                  </Column>
            ...
            <Plate role="console" gap="$2" padding={12} testID="campaign-status">
              ...
              <Row alignItems="flex-end" gap="$2">
                <RollingReadout
                  value={formatRaw(c.raisedWei, 18)}
                  hero
                  reducedMotion={reducedMotion}
                  testID="campaign-raised"
                />
              ...
              <Body tone="mute" size="caption">
                {t({
                  id: 'campaign.target',
                  message: '{p}% of the {t} ETN launch target · {n} contributors',
                  values: {
                    p: Math.round(c.fill * 100),
                    t: formatRaw(c.minEtnToLaunchWei, 18),
                    n: c.contributorCount,
                  },
                })}
              </Body>
            </Plate>
            ...
            <StatStrip
              ...
              cells={[
                {
                  label: t({ id: 'campaign.min', message: 'Min per wallet' }),
                  value: c.minContributionWei ? `${formatRaw(c.minContributionWei, 18)} ETN` : '—',
                },
                {
                  label: t({ id: 'campaign.max', message: 'Max per wallet' }),
                  value:
                    c.maxContributionWei && BigInt(c.maxContributionWei) > 0n
                      ? `${formatRaw(c.maxContributionWei, 18)} ETN`
                      : t({ id: 'campaign.nomax', message: 'No limit' }),
                },
                {
                  label: t({ id: 'campaign.yours', message: 'Your contribution' }),
                  value: `${formatRaw(c.contributedWei, 18)} ETN`,
                  tone: BigInt(c.contributedWei) > 0n ? 'arc' : 'ink',
                  testID: 'campaign-contributed',
                },
                {
                  label: t({ id: 'campaign.claimable', message: 'Claimable' }),
                  value:
                    BigInt(c.claimableTokensRaw) > 0n
                      ? `${formatRaw(c.claimableTokensRaw, c.token.decimals)} ${c.token.symbol}`
                      : '—',
                  tone: BigInt(c.claimableTokensRaw) > 0n ? 'ember' : 'ink',
                },
              ]}
              ...
              <Row gap="$2" flexWrap="wrap" testID="campaign-keys">
                {c.keys.includes('contribute') ? (
                  <Column flex={1} minWidth={140}>
                    <Key
                      label={t({ id: 'campaign.contribute', message: 'Contribute' })}
                      disabled={busy}
                      onPress={() => setSheet(true)}
                      testID="campaign-contribute"
                    />
                  </Column>
                ) : null}
                {c.keys.includes('claim_tokens') ? (
                  <Column flex={1} minWidth={140}>
                    <Key
                      label={t({ id: 'campaign.claimTokens', message: 'Claim tokens' })}
                      disabled={busy}
                      onPress={() =>
                        void run(() =>
                          engine.launchpad.claim({
                            accountId: active.id,
                            chainId,
                            pool,
                            kind: 'tokens',
                ...
                {c.keys.includes('claim_refund') ? (
                  <Column flex={1} minWidth={140}>
                    <Key
                      label={t({ id: 'campaign.claimRefund', message: 'Claim refund' })}
                      disabled={busy}
                      onPress={() =>
                        void run(() =>
                          engine.launchpad.claim({
                ...
                {c.keys.includes('claim_referral') ? (
                  <Column flex={1} minWidth={140}>
                    <Key
                      label={t({ id: 'campaign.claimReferral', message: 'Claim referral rewards' })}
                      kind="secondary"
                      size="compact"
                      disabled={busy}
                      onPress={() =>
                      ...
                      testID="campaign-claim-referral"
                    />
                  </Column>
                ) : null}
              </Row>
              ...
              <Row gap="$2" flexWrap="wrap" testID="campaign-links">
                {links.map((l) => (
                  <Pill
                    key={l.key}
                    label={l.label()}
                    icon={<Icon name={l.icon} size={14} color={paint.mute} />}
                    size="sm"
                    onPress={() => void host.openUrl?.(c.links[l.key] ?? '')}
                    testID={`campaign-link-${l.key}`}
                  />
                ))}
              </Row>
            ) : null}
            {c.shareLink && c.affiliatePercent > 0 ? (
              <Plate role="card" gap="$2" testID="campaign-referral">
                <Body size="caption">
                  {t({
                    id: 'campaign.ref',
                    message: 'Share your link — you earn {p}% of contributions',
                    values: { p: c.affiliatePercent },
                  })}
                </Body>
                <Row gap="$2" alignItems="center">
                  ...
                  <Pill
                    label={
                      copied
                        ...
                        : t({ id: 'copy', message: 'Copy' })
                    }
                    icon={<Icon name={copied ? 'check' : 'copy'} size={14} color={paint.mute} />}
                    size="sm"
                    onPress={() =>
                      void host
                        .copy?.(`https://electroswap.io/share/${c.shareLink}`)
                        .then(() => setCopied(true))
                    }
                    testID="campaign-copy-link"
                  />
                </Row>
                {BigInt(c.referralClaimableWei) > 0n ? (
                  <Body tone="ember" size="caption">
                    {t({
                      id: 'campaign.refEarned',
                      message: '{a} ETN in referral rewards to claim',
                      values: { a: formatRaw(c.referralClaimableWei, 18) },
                    })}
                  </Body>
                ) : null}
              </Plate>
      ...
      <Sheet
        ...
        title={t({ id: 'campaign.contribute.title', message: 'Contribute ETN' })}
        reducedMotion={reducedMotion}
        footer={
          <Key
            label={t({ id: 'campaign.contribute', message: 'Contribute' })}
      ...
      >
        <Column gap="$3">
          <Input
            ...
            onChange={setAmount}
            placeholder="0"
            label={t({ id: 'campaign.amount', message: 'Amount in ETN' })}
            autoFocus
            testID="campaign-amount"
          />
          {c?.minContributionWei ? (
            <Body tone="mute" size="caption">
              {t({
                id: 'campaign.range',
                message: 'Between {a} and {b} ETN per wallet',
                values: {
                  a: formatRaw(c.minContributionWei, 18),
                  b:
                    c.maxContributionWei && BigInt(c.maxContributionWei) > 0n
                      ? formatRaw(c.maxContributionWei, 18)
                      : '∞',
                },
              })}
            </Body>
          ...
          <Body tone="mute" size="caption">
            {t({
              id: 'campaign.contribute.body',
              message:
                'Contributions are native ETN. If the campaign reaches its target you claim tokens after launch; if not, you claim a refund.',
            })}
          </Body>
          {error ? <Body tone="burn">{error}</Body> : null}
        </Column>
