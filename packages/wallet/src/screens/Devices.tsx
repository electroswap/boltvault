...
export function Devices({ body }: { body: 'extension-popup' | 'extension-tab' | 'mobile' }) {
  ...
  const [remote, setRemote] = useState<{ outgoing: RemoteRequest[]; incoming: RemoteRequest[] }>({
    outgoing: [],
    incoming: [],
  })
  ...
  useEffect(() => {
    ...
    engine.vault.status().then(
      (s) => setHasVault(s.exists),
      () => undefined,
    )
    refreshWaiting()
  ...
  const scanOrPaste = async (): Promise<string> =>
    host.scanQr ? host.scanQr() : Promise.resolve(pasted)
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: inset, gap: 16 }} testID="devices">
      ...
      <Plate gap="$2">
        ...
        <Row gap="$2" alignItems="flex-end">
          ...
          <Key
            ...
            onPress={() =>
              run(async () => {
                await engine.sync.setDeviceLabel({ label: label.trim() })
                setLabel('')
              })
            }
          />
        ...
        <Body tone="mute" size="caption">
          {t({
            id: 'devices.what',
            message:
              'Paired devices share your address book, account names, custom and hidden tokens, per-site chains, settings, and watch-only and hardware accounts. Seeds and keys never sync — move a vault with the code below.',
          })}
        </Body>
      </Plate>
      ...
      {status?.pending ? (
        <Plate role="raised" gap="$3" testID="sas">
          ...
          <Body tone="mute" size="caption">
            {t({
              id: 'devices.sas.body',
              message: 'Both devices show a six-digit code. Confirm only if they match exactly.',
            })}
          </Body>
          ...
          <Row gap="$2">
            <Key
              label={t({ id: 'devices.sas.match', message: 'They match' })}
              onPress={() =>
                run(async () => {
                  await engine.sync.confirm()
                  setMode('idle')
                  setOffer(null)
                  setAnswer(null)
                })
            ...
            />
            <Key
              label={t({ id: 'devices.sas.no', message: 'They differ' })}
              kind="danger"
              onPress={() =>
                run(async () => {
                  await engine.sync.cancelPairing()
                  setMode('idle')
                  setOffer(null)
                  setAnswer(null)
                })
      ...
      {mode === 'idle' && !status?.pending ? (
        <Column gap="$2">
          <Key
            label={t({ id: 'devices.pair.new', message: 'Pair a new device' })}
            onPress={() =>
              run(async () => {
                const r = await engine.sync.createOffer({ relayUrl: host.relayUrl })
                setOffer(r.offer)
                setMode('offer')
              })
            }
          ...
          />
          <Key
            label={t({ id: 'devices.pair.join', message: 'Join from a code' })}
            kind="secondary"
      ...
      {mode === 'offer' && offer ? (
        <Plate gap="$3" testID="offer">
          <Body size="title">
            {t({ id: 'devices.offer.title', message: 'Scan this on the other device' })}
          </Body>
          ...
          {!host.scanQr ? (
            <Input
              ...
              onChange={setPasted}
              multiline
              placeholder={t({ id: 'devices.answer.paste', message: 'Paste the answer code' })}
            />
          ) : null}
          <Key
            label={
              host.scanQr
                ? t({ id: 'devices.scan', message: 'Scan answer' })
                : t({ id: 'devices.use', message: 'Use answer' })
            }
            onPress={() =>
              run(async () => {
                await engine.sync.completeOffer({ answer: await scanOrPaste() })
                setPasted('')
              })
            }
            testID="complete-offer"
          />
          <Key
      ...
      {mode === 'answer' ? (
        <Plate gap="$3" testID="answer">
          <Body size="title">
            {t({ id: 'devices.answer.title', message: 'Scan the other device’s code' })}
          </Body>
          {!host.scanQr ? (
            <Input
              value={pasted}
              onChange={setPasted}
              multiline
              placeholder={t({ id: 'devices.offer.paste', message: 'Paste the pairing code' })}
            />
          ) : null}
          {!answer ? (
            <Key
              label={
                host.scanQr
                  ? t({ id: 'devices.scan.code', message: 'Scan' })
                  : t({ id: 'devices.use.code', message: 'Use code' })
              }
              onPress={() =>
                run(async () => {
                  const r = await engine.sync.acceptOffer({ offer: await scanOrPaste() })
                  setAnswer(r.answer)
                  setPasted('')
                })
              }
            ...
            <>
              <Body tone="mute" size="caption">
                {t({
                  id: 'devices.answer.show',
                  message: 'Show this to the first device, then compare the codes.',
                })}
              </Body>
              <Column alignItems="center">
      ...
      */}
      {waiting.length > 0 ? (
        <Plate gap="$3" testID="devices-waiting">
          <Body size="title">
            {t({ id: 'devices.waiting', message: 'Waiting for you to confirm' })}
          </Body>
          <Body tone="mute" size="caption">
            {t({
              id: 'devices.waiting.body',
              message:
                'Names and tokens another device sent. Until you confirm one here, BoltVault will not treat it as a name it knows or a token it recognises — so a device that is not yours cannot put a name on an address or pass one token off as another.',
            })}
          </Body>
          {waiting.map((i) => (
            <Column
              key={`${i.collection}:${i.key}`}
              gap="$1"
              testID={`devices-waiting-${i.collection}`}
            >
              <Body>
                {i.collection === 'contact'
                  ? t({
                      id: 'devices.waiting.name',
                      message: 'Name: {label}',
                      values: { label: i.label },
                    })
                  : t({
                      id: 'devices.waiting.token',
                      message: 'Token: {label}',
                      values: { label: i.label },
                    })}
              </Body>
              ...
              <Body tone="mute" size="caption">
                {t({
                  id: 'devices.waiting.from',
                  message: 'from {device}, {date}',
                  values: { device: i.fromLabel, date: shortDate(i.at) },
                })}
              </Body>
              <Row gap="$2">
                <Key
                  label={t({ id: 'devices.waiting.confirm', message: 'Confirm' })}
                  size="compact"
                  onPress={() =>
                    run(async () => {
                      setWaiting(
                        await engine.sync.confirmIncoming({ collection: i.collection, key: i.key }),
                      )
                    })
                  }
                  testID={`devices-confirm-${i.collection}`}
                />
                <Key
                  label={t({ id: 'devices.waiting.reject', message: 'Remove it' })}
                  kind="danger"
                  size="compact"
                  onPress={() =>
                    run(async () => {
                      setWaiting(
                        await engine.sync.rejectIncoming({ collection: i.collection, key: i.key }),
                      )
                    })
                  }
                  testID={`devices-reject-${i.collection}`}
                />
              </Row>
            </Column>
          ))}
      ...
      <Plate gap="$2" testID="remote-requests">
        ...
        <Body tone="mute" size="caption">
          {t({
            id: 'devices.requests.body',
            message:
              'An account whose key lives on another paired device signs there: the request opens as a signing sheet on that device, and the answer comes back here. Only you can approve it, and only after reading what it signs.',
          })}
        </Body>
        ...
        {remote.outgoing.map((r) => (
          <Row
            ...
            alignItems="center"
            testID={`remote-out-${r.id}`}
          >
            <Body size="caption">
              {t({
                id: 'devices.requests.out',
                message: 'Waiting for a paired device · {k}',
                values: { k: r.kind },
              })}
            </Body>
            <Body
              tone="burn"
              size="caption"
              onPress={() => void engine.remote.cancel({ id: r.id })}
            >
        ...
        {remote.incoming.map((r) => (
          <Row
            ...
            justifyContent="space-between"
            alignItems="center"
            testID={`remote-in-${r.id}`}
          >
            <Body size="caption">
              {t({
                id: 'devices.requests.in',
                message: 'From {d} · {k} — open in the signing sheet',
                values: { d: r.from ?? '?', k: r.kind },
              })}
            </Body>
          </Row>
        ))}
      ...
      <Plate gap="$2" testID="paired">
        ...
        {status?.devices.map((d) => (
          <Row key={d.deviceId} justifyContent="space-between">
            ...
            <Body
              ...
              size="caption"
              onPress={() =>
                run(() => engine.sync.unpair({ deviceId: d.deviceId }).then(() => undefined))
              }
            >
        ...
        {status && status.devices.length > 0 ? (
          <Column gap="$2">
            <Key
              label={t({ id: 'devices.push', message: 'Send changes' })}
              kind="secondary"
              onPress={() =>
                run(async () => {
                  const r = await engine.sync.push()
                  setNote(
                    t({
                      id: 'devices.pushed',
            ...
            />
            <Key
              label={t({ id: 'devices.pull', message: 'Fetch changes' })}
              kind="secondary"
              onPress={() =>
                run(async () => {
                  const r = await engine.sync.pull()
                  setNote(
                    t({
                      id: 'devices.pulled',
                      message: 'Applied {n} records.',
                      values: { n: r.applied },
                    }),
                  )
                })
      ...
      */}
      <Plate gap="$3" testID="vault-move">
        ...
        <Body tone="mute" size="caption">
          {t({
            id: 'devices.move.body',
            message:
              'On the device that holds the vault, open Security and show the export. Read every frame of the moving code with the camera here, then type the six words it shows. BoltVault never shows those words on this side.',
          })}
        </Body>
        {hasVault ? (
          <Body tone="mute" size="caption">
            {t({
              id: 'devices.move.occupied',
              message:
                'This device already holds a vault, and a move only lands on one that does not. Do it while setting the device up, or remove this vault first.',
            })}
          </Body>
        ) : (
          <>
            {host.scanQr ? (
              <Key
                label={
                  frames.length === 0
                    ? t({ id: 'devices.move.scan', message: 'Read the moving code' })
                    : t({ id: 'devices.move.scan.more', message: 'Keep reading' })
                }
                kind="secondary"
                onPress={() => run(scanExport)}
                testID="vault-move-scan"
              />
            ) : (
              <Input
                value={movePasted}
                onChange={setMovePasted}
                multiline
                placeholder={t({
                  id: 'devices.move.paste',
                  message: 'Paste every frame, separated by spaces',
                })}
                testID="vault-move-paste"
              />
            )}
            {moveProgress.total > 0 ? (
              <Body tone={moveReady ? 'arc' : 'mute'} size="caption" testID="vault-move-progress">
                {t({
                  id: 'devices.move.frames',
                  message: '{seen} of {total} frames read.',
                  values: { seen: moveProgress.seen, total: moveProgress.total },
                })}
              </Body>
            ) : null}
            <Input
              ...
              autoCapitalize="none"
              placeholder={t({
                id: 'devices.move.code',
                message: 'The six words from the other device',
              })}
              sensitive="code"
            ...
            <Key
              ...
              onPress={() =>
                run(async () => {
                  const r = await engine.vault.importExport({
                    frames: moveFrames,
                    code: code.trim(),
                  ...
                  setNote(
                    t({
                      id: 'devices.move.done',
                      message:
                        'The vault is here — {n} accounts. Delete the export on the other device when you are sure.',
                      values: { n: r.accounts.length },
                    }),
                  )
                })
              }
