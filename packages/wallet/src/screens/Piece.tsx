...
export function Piece({
  ...
  useEffect(() => {
    let alive = true
    engine.nft
      .asset({ chainId, address, tokenId, ...(active ? { accountId: active.id } : {}) })
      .then(
        (a) => alive && setAsset(a),
  ...
  if (flow) {
    ...
    const done = kind.includes('buy')
      ? t({ id: 'piece.bought', message: 'It is yours' })
      : kind.includes('sign_order') && kind.includes('accept')
        ? t({ id: 'piece.sold', message: 'Sold' })
        : kind.includes('accept')
          ? t({ id: 'piece.sold', message: 'Sold' })
          : kind.includes('post_order')
            ? asset?.mine
              ? t({ id: 'piece.listed', message: 'Listed' })
              : t({ id: 'piece.offered', message: 'Offer made' })
            : kind.includes('cancel_order')
              ? t({ id: 'piece.cancelled', message: 'Cancelled' })
              : kind.includes('transfer')
                ? t({ id: 'piece.sent', message: 'Sent' })
                : t({ id: 'piece.done', message: 'Done' })
    return (
      <FlowPlate
        flow={flow}
        body={body}
        reducedMotion={reducedMotion}
        titles={{ working: t({ id: 'piece.working', message: 'Working…' }), done }}
        summary={asset?.name ?? null}
        onDone={dismiss}
        testID="nft-flow"
      />
    )
  ...
  return (
    <Column flex={1}>
            ...
            <>
              {asset ? (
                <Pill
                  ...
                  icon={
                    asset.collectionVerified ? (
                      <Icon name="check" size={14} color={paint.arc} />
                    ) : undefined
                  }
                  size="sm"
                  onPress={() => router.navigate('collection', { chainId, address })}
                  testID="piece-collection"
                />
              ) : null}
            </>
          }
        ...
        {asset ? (
          <>
            ...
            <SharedElement id={pieceSharedId(chainId, address, tokenId)}>
              <Artwork
                uri={asset.imageUrl ?? asset.smallImageUrl}
                label={asset.name}
                size={{ width, height: Math.round(width * 0.9) }}
                sweep
                reducedMotion={reducedMotion}
                testID="piece-art"
              />
            ...
            <Column gap={2}>
              ...
              <Body tone="mute" size="caption">
                {asset.owner
                  ? mine
                    ? t({ id: 'piece.yours', message: 'Yours' })
                    : t({
                        id: 'piece.owner',
                        message: 'Owned by {a}',
                        values: { a: shortAddress(asset.owner) },
                      })
                  : ''}
                {asset.rarityRank !== null
                  ? ` · ${t({ id: 'piece.rank', message: 'Rank #{r}', values: { r: asset.rarityRank } })}`
                  : ''}
              </Body>
            </Column>
            {asset.paysDividends ? (
              <Plate gap={2} testID="piece-dividends">
                <Body tone="ember" size="caption">
                  {asset.dividendsWei !== null && BigInt(asset.dividendsWei) > 0n
                    ? t({
                        id: 'piece.earns',
                        message: 'This piece earns marketplace dividends — {a} ETN unclaimed',
                        values: { a: formatRaw(asset.dividendsWei, 18) },
                      })
                    : t({
                        id: 'piece.earns.none',
                        message:
                          'This piece earns marketplace dividends (activate them after you hold it).',
                      })}
                </Body>
              </Plate>
            ) : null}
            <Row gap="$3" flexWrap="wrap" testID="piece-prices">
              <Column minWidth={90}>
                ...
                <Body tone={listing ? 'arc' : 'ink'}>
                  {listing?.priceEtn !== null && listing?.priceEtn !== undefined
                    ? `${listing.priceEtn} ETN`
                    : '—'}
                </Body>
              </Column>
              <Column minWidth={90}>
                ...
                <Body tone={best ? 'ember' : 'ink'}>
                  {best?.priceEtn !== null && best?.priceEtn !== undefined
                    ? `${best.priceEtn} WETN`
                    : '—'}
                </Body>
              </Column>
            ...
            {asset.custom ? (
              <Body tone="mute" size="caption" testID="piece-custom-note">
                {t({
                  id: 'piece.custom',
                  message:
                    'From a collection you added: read from the contract, not on the marketplace.',
                })}
              </Body>
            ) : null}
            {active ? (
              <Row gap="$2" flexWrap="wrap" testID="piece-keys">
                {mine ? (
                  <>
                    {!listing && !asset.custom ? (
                      <Key
                        label={t({ id: 'piece.list', message: 'List' })}
                        size="compact"
                        onPress={() => setSheet('list')}
                        testID="piece-list"
                      />
                    ) : null}
                    {listing && listing.actionable && listing.orderHash ? (
                      <Key
                        label={t({ id: 'piece.cancelListing', message: 'Cancel listing' })}
                        kind="secondary"
                        disabled={busy}
                        onPress={() =>
                          void run(() =>
                    ...
                    {best && best.actionable && best.orderHash ? (
                      <Key
                        label={t({ id: 'piece.accept', message: 'Accept offer' })}
                        disabled={busy}
                        onPress={() =>
                          void run(() =>
                            engine.nft.accept({
                    ...
                    <Key
                      label={t({ id: 'piece.transfer', message: 'Send' })}
                      kind="secondary"
                      size="compact"
                      onPress={() => setSheet('transfer')}
                      testID="piece-transfer"
                    />
                  </>
                ) : (
                  <>
                    {listing && listing.actionable ? (
                      <Key
                        label={t({ id: 'piece.buy', message: 'Buy' })}
                        disabled={busy}
                        onPress={() =>
                          void run(() =>
                            engine.nft.buy({ accountId: active.id, chainId, address, tokenId }),
                          )
                        }
                        testID="piece-buy"
                      />
                    ...
                    {!asset.custom ? (
                      <Key
                        label={t({ id: 'piece.offer', message: 'Offer' })}
                        kind="secondary"
                        size="compact"
                        onPress={() => setSheet('offer')}
                        testID="piece-offer"
                      />
                    ) : null}
                  </>
            ...
            {!asset.custom ? (
              <Pill
                label={t({ id: 'piece.web', message: 'On ElectroSwap' })}
                icon={<Icon name="external" size={14} color={paint.mute} />}
                size="sm"
                onPress={() =>
                  void host.openUrl?.(`https://app.electroswap.io/nfts/asset/${address}/${tokenId}`)
                }
              />
            ) : null}
          </>
      ...
      <Sheet
        ...
        title={
          sheet === 'list'
            ? t({ id: 'piece.list.title', message: 'List for sale' })
            : sheet === 'offer'
              ? t({ id: 'piece.offer.title', message: 'Make an offer' })
              : t({ id: 'piece.transfer.title', message: 'Send this piece' })
        }
        reducedMotion={reducedMotion}
        testID="piece-sheet"
      >
          ...
          {sheet === 'transfer' ? (
            <>
              <Input
                ...
                onChange={setTo}
                placeholder="0x…"
                label={t({ id: 'piece.transfer.to', message: 'To' })}
                testID="piece-transfer-to"
              />
              <Body tone="mute" size="caption">
                {t({
                  id: 'piece.transfer.body',
                  message:
                    'The recipient goes through the same address checks as a send. A collectible cannot be recalled.',
                })}
              </Body>
              <Key
                label={t({ id: 'piece.transfer', message: 'Send' })}
                disabled={busy || !/^0x[0-9a-fA-F]{40}$/.test(to.trim())}
                onPress={() =>
                  void run(() =>
                    engine.nft.transfer({
                      accountId: active?.id ?? '',
            ...
            <>
              <Input
                ...
                label={
                  sheet === 'list'
                    ? t({ id: 'piece.price', message: 'Price in ETN' })
                    : t({ id: 'piece.offer.price', message: 'Offer in WETN' })
                }
                testID="piece-price"
              />
              ...
              {sheet === 'list' && price && Number(price) > 0 ? (
                <Body tone="mute" size="caption" testID="piece-proceeds">
                  {t({
                    id: 'piece.proceeds',
                    message: 'You receive {a} ETN after the 3% marketplace fee{c}',
                    values: {
                      a: (
                        Number(price) *
                        (1 - 0.03 - (asset?.creatorFee?.basisPoints ?? 0) / 10_000)
                      )
                        .toFixed(4)
                        .replace(/\.?0+$/, ''),
                      c: asset?.creatorFee
                        ? ` and the ${(asset.creatorFee.basisPoints / 100).toFixed(1)}% creator royalty`
                        : '',
                    },
                  })}
                </Body>
              ...
              {sheet === 'offer' ? (
                <Body tone="mute" size="caption">
                  {t({
                    id: 'piece.offer.body',
                    message:
                      'Offers are made in WETN. If you are short, the wallet wraps ETN for you first. The owner can accept any time before it expires.',
                  })}
                </Body>
              ) : null}
              {error ? <Body tone="burn">{error}</Body> : null}
              <Key
                label={
                  sheet === 'list'
                    ? t({ id: 'piece.list', message: 'List' })
                    : t({ id: 'piece.offer', message: 'Offer' })
                }
