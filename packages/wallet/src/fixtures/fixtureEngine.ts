  ...
  if (scenario === 'funded' || scenario === 'empty' || scenario === 'keystone') {
    ...
    const swapQuote = (arg: { tokenIn: string; tokenOut: string; amountIn?: string; amountOut?: string; tradeType?: 'exactIn' | 'exactOut'; slippageBips?: number }): SwapQuote => {
      ...
      return {
        ...
        taxBips: 0,
        taxUnknown: false,
        fee: { bips: 40, tier: 1, name: 'Charge', amountRaw: fee.toString(), sink: SINK, source: 'config', onInput: false, nextTierAt: '136000000000000000000000', nextTierBips: 30 },
    ...
    const offer = { type: 'BID' as const, status: 'VALID' as const, priceEtn: 36.5, priceRaw: '36500000000000000000', orderHash: '0xbid1', maker: OTHER, createdAt: Math.floor(FIXED_NOW / 1000) - 3_600, endAt: Math.floor(FIXED_NOW / 1000) + 5 * 86_400, actionable: true, proceedsTo: address, paysPreviousOwner: false }
    const listing = { type: 'LISTING' as const, status: 'VALID' as const, priceEtn: 42, priceRaw: '42000000000000000000', orderHash: '0xlist1', maker: address, createdAt: Math.floor(FIXED_NOW / 1000) - 7_200, endAt: Math.floor(FIXED_NOW / 1000) + 6 * 86_400, actionable: true, proceedsTo: address, paysPreviousOwner: false }
    ...
    engine.host.override('explore', {
      ...
      /*
        ...
        The timestamps are offsets from the REAL clock, not from `FIXED_NOW`
        like the activity rows above: these render as "2m ago", so a baseline
        is only stable if the rows move with the clock that renders them. The
        distances are deliberately far from a boundary, so a shot taken a few
        seconds after the previous one still reads the same.
      */
