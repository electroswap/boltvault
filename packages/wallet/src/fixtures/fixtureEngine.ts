...
export type FixtureScenario =
  'fresh' | 'locked' | 'unlocked' | 'funded' | 'empty' | 'connect' | 'sign' | 'keystone'
...
const heads: HeadSource = {
  blockNumber: async (chainId) => BigInt(chainId === 52014 ? 15_212_345 : 21_000_000),
}
...
const LEGENDS_ART = {
  ...
  piece: (tokenId: string): string =>
    `${ART_BASE}/nfts/0x31cbb613D14cc85Cf3A8889007562E4B5cE9518b/${tokenId}_medium.webp`,
} as const
...
export async function createFixtureEngine(
  ...
  */
  const FIXTURE_NAMES: Record<string, string> = {
    '0x2222222222222222222222222222222222222222': 'shop.etn',
  }
  ...
  if (scenario !== 'fresh') {
    const { seedId, accounts } = await engine.engine.vault.import({
      mnemonic: MNEMONIC,
      password: PASSWORD,
    })
    ...
    if (
      ...
      await engine.engine.vault.confirmBackup({
        seedId,
        answers: quiz.positions.map((position) => ({ position, word: words[position - 1] ?? '' })),
      })
    }
    if (scenario === 'funded' && account) {
      await engine.sites.registry.connect(SITE, {
      ...
      await engine.engine.accounts.addWatch({
        address: '0x9a2c4f1e8b7d3c6a5e4f2d1c0b9a8e7d6c5b41e0',
        label: 'Cold storage',
      })
      await engine.engine.accounts.addHardware({
        kind: 'ledger',
        address: '0x7b63c5e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7',
        path: "m/44'/60'/0'/0/3",
        deviceId: 'fx-ledger',
        label: 'Ledger #3',
      })
    ...
    if (scenario === 'connect') {
      await engine.approvals.create({
        ...
        payload: {
          kind: 'connect',
          requestedChainId: 52014,
          reconnect: false,
          firstTime: true,
          clientRequestId: 'fixture-connect',
        },
      })
    }
      ...
      await engine.approvals.create({
        ...
        payload: {
          ...
          assessment: {
            ...
            rules: [
              {
                ...
                detail:
                  'This lets 0x2222…2222 spend an unlimited amount of 0x1111…1111. BoltVault does not recognise the spender.',
              },
              {
                code: 'SIM_INCOMPLETE',
                severity: 'warn',
            ...
            statements: [
              {
                text: 'Allow 0x2222…2222 to move an unlimited amount of 0x1111…1111',
                tone: 'warn',
              },
            ],
            changes: [],
  ...
  if (scenario === 'funded' || scenario === 'empty' || scenario === 'keystone') {
    ...
    const address =
      (await engine.engine.accounts.list())[0]?.address ??
    ...
    const snap =
      scenario === 'empty'
        ...
        : full
    const tokens: TokenView[] = snap.rows.map((r) => ({
      chainId: r.chainId,
      address: r.address,
      symbol: r.symbol,
      name: r.name,
      decimals: r.decimals,
      logoUri: r.logoUri,
      source: r.address === 'native' ? 'native' : 'list',
      pinned: r.pinned,
      hidden: false,
      tags: [],
    }))
    const activity: ActivityEntry[] = [
      {
        ...
        nonce: 3,
        submittedAt: FIXED_NOW - 3_600_000,
        origin: 'internal:send',
        category: 'SEND',
        statements: ['Send 1,000 ETN to 0x2222…2222'],
        riskCodes: ['RECIPIENT_FIRST_TIME'],
        status: 'confirmed',
        blockNumber: 15_212_100,
        token: 'native',
      },
      {
        id: 'fx-2',
        ...
        value: '250000000',
        nonce: null,
        submittedAt: FIXED_NOW - 7_200_000,
        origin: null,
        category: 'RECEIVE',
        statements: ['Received USDC from 0x3333…3333'],
        riskCodes: [],
        status: 'confirmed',
        blockNumber: 15_211_000,
        token: '0x3187deAd7A2Bd6770F5Fe81495D1B715926AAe6e',
      },
      {
        id: 'fx-3',
        ...
        chainId: 52014,
        accountId,
        to: '0x043fAa1b5C5FC9a7dc35171f290c29ECDE0cCff1',
        value: '0',
        nonce: 4,
        submittedAt: FIXED_NOW - 600_000,
        origin: 'https://app.electroswap.io',
        category: 'APPROVE',
        statements: ['Allow Permit2 to move up to 5,000 BOLT'],
        riskCodes: [],
        status: 'pending',
        blockNumber: null,
        token: '0x043fAa1b5C5FC9a7dc35171f290c29ECDE0cCff1',
      },
    ]
    const allowances: AllowanceView[] = [
      {
        ...
        tokenSymbol: 'BOLT',
        decimals: 18,
        spender: '0x012ff228Aa9Fec4dBEE6Cd704072749AF077b617',
        spenderName: 'Permit2',
        known: true,
        standard: 'erc20',
        amount: 'unlimited',
      ...
      },
      {
        ...
        tokenSymbol: 'USDC',
        decimals: 6,
        spender: '0x2c12c8F15637b7A182DEc202816148A5E767DCEC',
        spenderName: 'ElectroSwap Universal Router',
        known: true,
        standard: 'permit2',
        amount: '1248000000',
        expiration: Math.floor(FIXED_NOW / 1000) + 1_800,
      },
      {
        chainId: 52014,
        token: '0x3187deAd7A2Bd6770F5Fe81495D1B715926AAe6e',
    ...
    engine.host.override('tokens', {
      ...
      get: {
        ...
        handler: async (arg) =>
          tokens.find(
            (t) => t.address.toLowerCase() === (arg as { address: string }).address.toLowerCase(),
          ) ?? null,
      },
      search: {
        ...
        handler: async (arg) =>
          tokens.filter((t) =>
            t.symbol.toLowerCase().includes((arg as { query: string }).query.toLowerCase()),
          ),
      },
    })
    ...
    engine.host.override('activityScan', {
      scan: {
        input: AccountArg,
        handler: async () => ({ added: 0, fromBlock: 15_212_000, toBlock: 15_212_345 }),
      },
    })
    engine.host.override('allowances', {
      cached: {
        input: AccountArg,
        handler: async () => ({ rows: allowances, at: FIXED_NOW - 30_000 }),
      },
    ...
    engine.host.override('send', {
      quote: {
        ...
        handler: async (arg) => {
          ...
          const row =
            snap.rows.find((r) => r.address.toLowerCase() === token.toLowerCase()) ?? snap.rows[0]
          const ok = /^0x[0-9a-fA-F]{40}$/.test(to) && Number(amount) > 0
          return {
            to: /^0x[0-9a-fA-F]{40}$/.test(to) ? to : null,
            name: null,
            token,
            symbol: row?.symbol ?? 'ETN',
            decimals: row?.decimals ?? 18,
            amountRaw: '0',
            balanceRaw: row?.raw ?? '0',
            maxRaw: row?.raw ?? '0',
            max: row?.quantity ?? '0',
            ...
            problems: ok
              ...
              : [
                  Number(amount) > 0
                    ? 'Enter a full address or a name.'
    ...
    const tierView: HolderTier = {
      chainId: 52014,
      bips: 40,
      tier: 1,
      name: 'Charge',
      nextTierName: 'Magneto',
      score: '18400000000000000000000',
      nextTierAt: '136000000000000000000000',
      nextTierBips: 30,
      source: 'config',
      sink: SINK,
      schedule: '0x00000000000000000000000000000000000005c4',
      breakdown: { wallet: '18400000000000000000000', farm: '0', dyno: '0' },
    }
    const schedule: FeeScheduleView = {
      ...
      baseBips: 50,
      tiers: [
        { name: 'Charge', minScore: '13600000000000000000000', bips: 40 },
        { name: 'Magneto', minScore: '136000000000000000000000', bips: 30 },
        { name: 'Turbine', minScore: '680000000000000000000000', bips: 20 },
        { name: 'Reactor', minScore: '1360000000000000000000000', bips: 10 },
      ],
      dynoWeight: '828590000000000000000',
      dynoWeightSource: 'average',
      countFarmBolt: true,
      source: 'config',
      sink: SINK,
      address: '0x00000000000000000000000000000000000005c4',
    }
    const swapQuote = (arg: {
      tokenIn: string
      tokenOut: string
      ...
      const inRow =
        snap.rows.find((r) => r.address.toLowerCase() === arg.tokenIn.toLowerCase()) ?? snap.rows[0]
      const outRow =
        snap.rows.find((r) => r.address.toLowerCase() === arg.tokenOut.toLowerCase()) ??
        snap.rows[2]
      const decIn = inRow?.decimals ?? 18
      const decOut = outRow?.decimals ?? 6
      const amountIn =
        BigInt(Math.round(Number(arg.amountIn || '0') * 1e6)) *
        10n ** BigInt(Math.max(0, decIn - 6))
      // A fixed rate so the screenshot is stable: 1 in = 0.00296 out (ETN → USDC).
      const amountOut =
        (amountIn * 296n * 10n ** BigInt(decOut)) / (100_000n * 10n ** BigInt(decIn))
      ...
      return {
        ...
        route: {
          ...
          hops: [
            {
              kind: 'v3',
              tokenIn: '0x138DAFbDA0CCB3d8E39C19edb0510Fc31b7C1c77',
              tokenOut: USDC,
              fee: 3000,
            },
          ],
    ...
    engine.host.override('holder', {
      ...
      addresses: {
        ...
        handler: async () => ({ sink: SINK, schedule: schedule.address }),
      },
    })
      ...
      quote: {
        input: z
          .object({
            ...
            tokenOut: z.string(),
            amountIn: z.string(),
            slippageBips: z.number().optional(),
          })
          .passthrough(),
        handler: async (arg) =>
          swapQuote(
            arg as { tokenIn: string; tokenOut: string; amountIn: string; slippageBips?: number },
          ),
      },
        ...
        handler: async () => [
          {
            ...
            decimalsIn: 18,
            decimalsOut: 6,
            amountInExact: '5000000000000000000000',
            amountOutMin: '1100000000',
            amountInRemaining: '5000000000000000000000',
            amountOutFilled: '0',
            unwrapOutput: false,
            createdAt: Math.floor(FIXED_NOW / 1000) - 3_600,
            expiresAt: Math.floor(FIXED_NOW / 1000) + 6 * 86_400,
      ...
      quote: {
        ...
        handler: async (arg) => {
          ...
          return {
            ...
            tokenOut: a.tokenOut,
            symbolIn: q.symbolIn,
            symbolOut: q.symbolOut,
            decimalsIn: q.decimalsIn,
            decimalsOut: q.decimalsOut,
            amountInRaw: q.amountInRaw,
            balanceInRaw: q.balanceInRaw,
            minOutRaw: minOut.toString(),
            targetRate: target,
            marketRate: 0.00296,
            distancePct: target ? (target / 0.00296 - 1) * 100 : null,
            durationSeconds: a.durationSeconds,
            platformFeeBips: 10,
            steps: ['approve', 'permit', 'submit'],
            ok: q.ok && minOut > 0n,
            problems: q.ok && minOut > 0n ? [] : ['Enter the least you will accept.'],
          }
        },
      },
    })
    ...
    const legendsCollection: CollectionView = {
      chainId: 52014,
      address: LEGENDS,
      ...
      sales: 4,
      description:
        'The flagship ElectroSwap collection. Every Legend shares a third of the marketplace fees.',
      verified: true,
      standard: 'ERC721',
      totalSupply: 500,
      imageUrl: art ? LEGENDS_ART.logo : null,
      bannerUrl: art ? LEGENDS_ART.banner : null,
      creatorFee: { payoutAddress: OTHER, basisPoints: 500 },
      floorEtn: 40,
      volume24hEtn: 128,
      totalVolumeEtn: 41_208,
      owners: 212,
      listed: 31,
      percentListed: 6.2,
      traits: [{ name: 'Element', values: ['Volt', 'Arc', 'Plasma'] }],
      paysDividends: true,
      starred: false,
      owned: 2,
      mint: {
    ...
    const voltsCollection: CollectionView = {
      chainId: 52014,
    ...
    const piece = (tokenId: string, extra: Partial<AssetView> = {}): AssetView => ({
      chainId: 52014,
      address: LEGENDS,
      tokenId,
      name: `Legend #${tokenId}`,
      description: 'A Volt-class Legend, struck in the first storm.',
      imageUrl: artOf(tokenId),
      smallImageUrl: artOf(tokenId),
      animationUrl: null,
      mediaType: 'IMAGE',
      owner: address,
      mine: true,
      standard: 'ERC721',
      collectionName: 'Electric Legends',
      collectionVerified: true,
      collectionImageUrl: art ? LEGENDS_ART.logo : null,
      creatorFee: { payoutAddress: OTHER, basisPoints: 500 },
      suspicious: false,
      rarityRank: Number(tokenId) * 3,
      traits: [
        { name: 'Element', value: 'Volt', rarity: 0.12 },
        { name: 'Charge', value: 'High', rarity: 0.3 },
      ],
      lastPriceEtn: 38,
      listing: null,
      bestBid: null,
      bids: [],
      dividendsWei: '310000000000000000',
      paysDividends: true,
      ...extra,
    })
    const offer = {
      ...
      priceEtn: 36.5,
      priceRaw: '36500000000000000000',
      orderHash: '0xbid1',
      maker: OTHER,
      createdAt: Math.floor(FIXED_NOW / 1000) - 3_600,
      endAt: Math.floor(FIXED_NOW / 1000) + 5 * 86_400,
      actionable: true,
    }
    const listing = {
      ...
      status: 'VALID' as const,
      priceEtn: 42,
      priceRaw: '42000000000000000000',
      orderHash: '0xlist1',
      maker: address,
      createdAt: Math.floor(FIXED_NOW / 1000) - 7_200,
      endAt: Math.floor(FIXED_NOW / 1000) + 6 * 86_400,
      actionable: true,
    }
    const owned: AssetView[] = [
      piece(ownedIds[0] as string, { bids: [offer], bestBid: offer }),
      piece(ownedIds[1] as string, { listing }),
      piece(ownedIds[2] as string, { rarityRank: 9, dividendsWei: '0' }),
    ]
      ...
      collections: [
        {
          address: LEGENDS,
          name: 'Electric Legends',
          logoUrl: art ? LEGENDS_ART.logo : null,
          balance: 3,
          floorEtn: 40,
        },
      ],
    ...
    const legendsStatus: LegendsStatus = {
      accountId,
      chainId: 52014,
      collection: LEGENDS,
      distributor: '0xc4065B310d64a02Ac4BF43CFd35C5Fe1A42811ea',
      ownedTokenIds: ownedIds,
      registeredTokenIds: ['12', '13'],
      unregisteredTokenIds: ['41'],
      claimableWei: '3210000000000000000',
      bestClaimWei: '5000000000000000000',
      vesselLevel: 0.64,
      lifetimePaidWei: '41208000000000000000000',
      activeTokenCount: 300,
      shareOfNextFee: 0.00222,
      dividendsEnabled: true,
      mint: {
        mintable: true,
        priceWei: '105000000000000000000',
        mintableCount: 3,
        totalSupply: 500,
      },
      observedAt: FIXED_NOW,
    ...
    const farm: FarmView = {
      ...
      version: 2,
      name: 'ETN/USDC',
      poolAddr: '0x7777777777777777777777777777777777777777',
      token0: '0x138DAFbDA0CCB3d8E39C19edb0510Fc31b7C1c77',
      token1: USDC,
      symbol0: 'ETN',
      symbol1: 'USDC',
      decimals0: 18,
      ...
      baseApy: 41.2,
      thirdPartyApy: 3.1,
      thirdParty: { token: '0x8888888888888888888888888888888888888888', symbol: 'ZAP' },
      farmerCount: 88,
      position: {
        ...
        boltDeposited: '50000000000000000000000',
        startingBlock: 12_058_000,
        blocksServed: 3_153_600,
        pendingRewards: '12400000000000000000',
        pendingThirdParty: '0',
        fees0: '0',
        fees1: '0',
        at2x: FIXED_NOW + 61 * 86_400_000,
        at25x: FIXED_NOW + 183 * 86_400_000,
        nextStair: {
          bolt: '100000000000000000000000',
          multiplier: 11_500,
          more: '50000000000000000000000',
        },
        amount0: '410000000000000000000000',
        amount1: '1213000000',
      },
    }
    const farm2: FarmView = {
      ...farm,
      id: 1,
      version: 3,
      name: 'BOLT/ETN',
      token0: BOLT,
      token1: '0x138DAFbDA0CCB3d8E39C19edb0510Fc31b7C1c77',
      symbol0: 'BOLT',
      symbol1: 'ETN',
      decimals0: 18,
      decimals1: 18,
    ...
    const campaign: CampaignView = {
      ...
      status: 'ACTIVE',
      phase: 'live',
      token: { name: 'Zap Protocol', symbol: 'ZAP', decimals: 18, address: null },
      creator: OTHER,
      creatorName: 'zap.etn',
      logoUrl: null,
      bannerUrl: null,
      description: 'Zap is a lightning-fast payments layer on Electroneum.',
      links: {
        website: 'https://zap.example',
        twitter: null,
        discord: null,
        telegram: 'https://t.me/zap',
      },
      starts: Math.floor(FIXED_NOW / 1000) - 86_400,
      ends: Math.floor(FIXED_NOW / 1000) + 3 * 86_400,
      raisedWei: '3120000000000000000000',
      minEtnToLaunchWei: '5000000000000000000000',
      maxContributionWei: '500000000000000000000',
      minContributionWei: '1000000000000000000',
      fill: 0.624,
      contributorCount: 63,
      affiliatePercent: 5,
      shareLink: 'zap7k',
      contributedWei: '250000000000000000000',
      claimed: false,
      claimableTokensRaw: '0',
      referralClaimableWei: '0',
      keys: ['contribute'],
      starred: true,
    }
    const campaign2: CampaignView = {
      ...campaign,
      pool: '0x9999999999999999999999999999999999999998',
      status: 'PENDING',
      phase: 'upcoming',
      token: { name: 'Nimbus', symbol: 'NIM', decimals: 18, address: null },
      starts: Math.floor(FIXED_NOW / 1000) + 2 * 86_400,
      ends: Math.floor(FIXED_NOW / 1000) + 9 * 86_400,
      raisedWei: '0',
      fill: 0,
      contributorCount: 0,
      contributedWei: '0',
    ...
    const positions: Positions = {
      ...
      campaigns: [campaign],
      accessories: [
        { kind: 'dividends', text: '3.21 ETN in dividends to claim', target: 'legends' },
        { kind: 'collect', text: '12.4 DYNO to collect', target: 'farm:1' },
      ],
      observedAt: FIXED_NOW,
    }
    const watch: WatchItem[] = [
      ...
      },
      {
        ...
        below: null,
        onLive: true,
        addedAt: FIXED_NOW - 3_600_000,
        lastValue: 1,
      },
    ]
    const offersInbox: OffersInbox = {
      received: [{ asset: owned[0] as AssetView, offer }],
      made: [
        {
          address: '0x8888888888888888888888888888888888888888',
          tokenId: '404',
          name: 'Volt #404',
          imageUrl: null,
          collectionName: 'Volts',
          offer: {
            ...offer,
            priceEtn: 2.5,
            priceRaw: '2500000000000000000',
            orderHash: '0xbid2',
            maker: address,
          },
          expiresAt: Math.floor(FIXED_NOW / 1000) + 2 * 86_400,
        },
      ],
      obligationWei: '2500000000000000000',
      wetnBalanceWei: '4000000000000000000',
    }
    ...
    await engine.notifications.push({
      id: 'offer:fixture',
      kind: 'offer',
      title: 'Offer on Electric Legend #77',
      body: '1,400 ETN from 0x9a2c…41e0',
      target: 'offers',
    })
    await engine.notifications.push({
      id: 'above:token:fixture',
      kind: 'alert',
      title: 'BOLT crossed $0.19',
      body: 'Your alert at $0.18 fired.',
      target: `token:${BOLT}`,
    })
    engine.host.override('explore', {
        ...
        handler: async (arg) => {
          ...
          if (a === 'native') {
            return {
              ...
              spam: false,
              logoUrl: null,
              description: "Electroneum's own coin, and the gas of every transaction on the chain.",
              homepageUrl: 'https://electroneum.com',
              twitterUrl: null,
              telegramUrl: null,
              sparkline: fixturePrices('1D').points,
            }
          }
          ...
          return {
            ...
            logoUrl: null,
            description:
              'BOLT is the ElectroSwap utility token. Holding it lowers the wallet fee on every swap, boosts farm rewards through the multiplier stairs, and puts you on the launchpad allowlist. Supply is fixed; a share of every marketplace fee buys BOLT back for the Legends vault.',
            homepageUrl: 'https://electroswap.io',
            twitterUrl: 'https://x.com/electroswap',
            telegramUrl: 'https://t.me/electroswap',
            sparkline: fixturePrices('1D').points,
          }
        },
      },
      priceHistory: {
        input: Any,
        handler: async (arg) =>
          (arg as { address: string }).address.toLowerCase() === BOLT.toLowerCase()
            ? fixturePrices((arg as { duration: '1D' | '1W' | '1M' | '1Y' }).duration)
            : null,
      },
      liquidity: {
        input: Any,
        handler: async (arg) =>
          (arg as { address: string }).address.toLowerCase() === BOLT.toLowerCase()
            ? { chainId: 52014, address: BOLT, lockedPct: 87, lockCount: 2 }
            : null,
      },
      ...
      collection: {
        input: Any,
        handler: async (arg) =>
          (arg as { address: string }).address.toLowerCase() === LEGENDS.toLowerCase()
            ? legendsCollection
            : voltsCollection,
      },
      search: {
        input: Any,
        handler: async (arg) => ({
          tokens: exploreTokens.filter((x) =>
            x.symbol.toLowerCase().includes(String((arg as { query: string }).query).toLowerCase()),
          ),
          collections: [],
        }),
      },
    ...
    engine.host.override('nft', {
        ...
        handler: async () => ({
          assets: [
            ...
            piece('77', {
              owner: OTHER,
              mine: false,
              listing: { ...listing, maker: OTHER, priceEtn: 44, priceRaw: '44000000000000000000' },
              dividendsWei: '0',
            }),
            piece('78', { owner: OTHER, mine: false, dividendsWei: '0' }),
          ],
          total: 500,
          next: null,
        }),
      ...
      asset: {
        input: Any,
        handler: async (arg) =>
          owned.find((a) => a.tokenId === (arg as { tokenId: string }).tokenId) ??
          piece(String((arg as { tokenId: string }).tokenId), {
            owner: OTHER,
            mine: false,
            listing: { ...listing, maker: OTHER, priceEtn: 44, priceRaw: '44000000000000000000' },
            dividendsWei: '0',
        ...
        handler: async () => [
          {
            address: LEGENDS,
            tokenId: art ? '24' : '13',
            name: art ? 'Legend #24' : 'Legend #13',
            imageUrl: artOf(art ? '24' : '13'),
            type: 'LISTING' as const,
            from: address,
            to: null,
            hash: null,
            priceEtn: 42,
            timestamp: Math.floor(FIXED_NOW / 1000) - 7_200,
          },
          {
            address: LEGENDS,
            tokenId: art ? '6' : '9',
            name: art ? 'Legend #6' : 'Legend #9',
            imageUrl: artOf(art ? '6' : '9'),
            type: 'SALE' as const,
            from: OTHER,
            to: address,
            hash: `0x${'d4'.repeat(32)}`,
            priceEtn: 38,
            timestamp: Math.floor(FIXED_NOW / 1000) - 86_400,
          },
        ],
      },
      ...
      previewCollection: {
        ...
        handler: async (arg) => ({
          chainId: 52014,
          address: (arg as { address: string }).address,
          name: 'Volt Punks',
          symbol: 'VPUNK',
    ...
    engine.host.override('farm', {
      ...
      farm: {
        input: Any,
        handler: async (arg) => ((arg as { farmId: number }).farmId === 1 ? farm2 : farm),
      },
      quoteDeposit: {
        input: Any,
        handler: async () => ({
          farmId: 0,
          amount0Raw: '10000000000000000000',
          amount1Raw: '29600',
          boltRaw: '0',
          nativeSide: 0,
          liquidityAdded: '1000000000000000',
          multiplierBefore: 17_500,
          multiplierAfter: 17_320,
          boltStair: null,
          steps: ['approve', 'deposit'],
          ok: true,
          problems: [],
        }),
      },
      quoteWithdraw: {
        input: Any,
        handler: async (arg) => {
          const pct = (arg as { percent: number }).percent
          return {
            farmId: 0,
            liquidityRaw: '50000000000000000000',
            percent: pct,
            amount0Raw: String((BigInt(410_000n * 10n ** 18n) * BigInt(Math.round(pct))) / 100n),
            amount1Raw: String((1_213_000_000n * BigInt(Math.round(pct))) / 100n),
            rewardsRaw: '12400000000000000000',
            thirdPartyRaw: '0',
            fees0Raw: '0',
            fees1Raw: '0',
            boltReturnedRaw: pct >= 100 ? '50000000000000000000000' : '0',
            keepsMultiplier: pct < 100,
            ok: true,
            problems: [],
          }
        },
    ...
    engine.host.override('launchpad', {
      ...
      detail: {
        input: Any,
        handler: async (arg) =>
          (arg as { pool: string }).pool.toLowerCase() === POOL ? campaign : campaign2,
      },
    })
    ...
    const transfers: BridgeStatus[] = [
      {
        ...
        decimals: 6,
        recipient: address,
        originHash: `0x${'d1'.repeat(32)}`,
        messageId: `0x${'ee'.repeat(32)}`,
        destinationHash: null,
        state: 'dispatched',
        startedAt: FIXED_NOW - 90_000,
        updatedAt: FIXED_NOW - 15_000,
        scanFrom: 21_000_000,
      },
      {
        id: `0x${'d2'.repeat(32)}`,
        ...
        decimals: 6,
        recipient: address,
        originHash: `0x${'d2'.repeat(32)}`,
        messageId: `0x${'ef'.repeat(32)}`,
        destinationHash: `0x${'d3'.repeat(32)}`,
        state: 'delivered',
        startedAt: FIXED_NOW - 86_400_000,
        updatedAt: FIXED_NOW - 86_000_000,
        scanFrom: null,
      },
    ]
    engine.host.override('bridge', {
      routes: {
        ...
        handler: async (arg) =>
          routes.filter(
            (r) =>
              r.fromChainId === (arg as { fromChainId: number }).fromChainId &&
              (!(arg as { token?: string }).token ||
                r.token.toLowerCase() === ((arg as { token?: string }).token ?? '').toLowerCase()),
          ),
      },
      quote: {
        input: Any,
        handler: async (arg) => {
          ...
          const r =
            routes.find(
              (x) => x.toChainId === a.toChainId && x.token.toLowerCase() === a.token.toLowerCase(),
            ) ?? routes[0]
          const amountRaw = BigInt(Math.round(Number(a.amount || '0') * 1e6))
          const problems =
            amountRaw <= 0n
              ? ['Enter an amount above zero.']
              : amountRaw > 1_248_000_000n
                ? ['Not enough USDC.']
                : []
          return {
            fromChainId: 52014,
            toChainId: r?.toChainId ?? 8453,
            symbol: r?.symbol ?? 'USDC',
            token: a.token,
            decimals: 6,
            amountRaw: amountRaw.toString(),
            balanceRaw: '1248000000',
            recipient: a.recipient ?? address,
            gasQuoteWei: '1250000000000000000',
            txFeeWei: '220000000000000',
            feeSymbol: 'ETN',
            etaMinutes: r?.toChainId === 1 ? 20 : 5,
            steps: ['submit'],
            recipientCode: { origin: false, destination: false },
      ...
      status: {
        input: Any,
        handler: async (arg) => transfers.find((x) => x.id === (arg as { id: string }).id) ?? null,
      },
    })
  }
  if (scenario === 'keystone') {
    ...
    const address =
      ...
      '0x0000000000000000000000000000000000000000'
    const frames = [
      'UR:ETH-SIGN-REQUEST/1-2/LPADAOCFADHDCYWEHGLGHDCSOEADTPDAGDWEDRGSBBFTMOAOCXAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAE',
      'UR:ETH-SIGN-REQUEST/2-2/LPAOAOCFADHDCYWEHGLGHDCSOEADTPDAGDWEDRGSBBFTMOAOCXAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAEAE',
    ]
    engine.host.override('hardware', {
      keystonePending: {
        handler: async () => [
          {
            id: 'fx-keystone',
...
function fixturePrices(duration: '1D' | '1W' | '1M' | '1Y'): {
  chainId: number
  address: string
