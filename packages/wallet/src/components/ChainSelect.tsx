...
export function ChainSelectPill({ chainId, label, onPress, size = 'md', testID }: { chainId: ChainChoice | null; label: string; onPress: () => void; size?: 'sm' | 'md'; testID?: string }) {
  const icon = chainId === 'all' ? <Icon name="globe" size={size === 'sm' ? 14 : 16} color={paint.arc} /> : chainId !== null ? <ChainMark chainId={chainId} size={size === 'sm' ? 14 : 16} /> : undefined
  return <Pill label={label} icon={icon} chevron tone="ink" size={size} onPress={onPress} accessibilityLabel={t({ id: 'chain.select.a11y', message: 'Chain: {c}', values: { c: label } })} testID={testID} />
}

/** The chain's mark and name as a plain caption — for screens that only state their chain. */
export function ChainCaption({ chainId, name, testID }: { chainId: number; name: string; testID?: string }) {
  return (
...
export function ChainSheet({ open, onClose, title, options, selected, onSelect, footer, reducedMotion = false, testID = 'chain-sheet', rowTestID = (id) => `chain-${id}` }: { open: boolean; onClose: () => void; title: string; options: readonly ChainOption[]; selected: ChainChoice | null; onSelect: (id: ChainChoice) => void; footer?: React.ReactNode; reducedMotion?: boolean; testID?: string; rowTestID?: (id: ChainChoice) => string }) {
  return (
    <Sheet open={open} onClose={onClose} title={title} reducedMotion={reducedMotion} {...(footer ? { footer } : {})} testID={testID}>
      <Column gap={2}>
        {options.map((o) => {
          const isSelected = o.id === selected
          const off = o.disabled === true
          return (
            <Pressable key={String(o.id)} onPress={off ? undefined : () => onSelect(o.id)} accessibilityRole="button" accessibilityState={{ selected: isSelected, disabled: off }} accessibilityLabel={o.name} testID={rowTestID(o.id)} style={{ minHeight: 52, justifyContent: 'center', opacity: off ? 0.6 : 1 }}>
              <Row gap="$3" alignItems="center" paddingHorizontal={4} paddingVertical={6} borderRadius={12} backgroundColor={isSelected ? '$glass' : 'transparent'}>
                {o.id === 'all' ? (
                  <Row width={28} height={28} borderRadius={14} alignItems="center" justifyContent="center" backgroundColor="$glassRaised">
                ...
                {off && o.action ? (
                  <Pill label={o.action.label} size="sm" onPress={o.action.onPress} testID={o.action.testID} />
                ) : (
                  <>
                    {o.value ? (
...
export function ManageNetworksKey({ onPress, testID = 'chain-networks' }: { onPress: () => void; testID?: string }) {
  return <Key label={t({ id: 'home.scope.manage', message: 'Manage networks' })} kind="secondary" size="compact" onPress={onPress} testID={testID} />
}
