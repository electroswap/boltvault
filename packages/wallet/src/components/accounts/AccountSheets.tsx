...
export function AccountDetailsSheet({
  ...
  const rows: Array<{ label: string; value: string }> = [
    ...
    ...(seed
      ? [{ label: t({ id: 'acct.details.wallet', message: 'Wallet' }), value: seed.label }]
      : []),
    ...(derivationLabel(account)
      ? [
          {
            label: t({ id: 'acct.details.derivation', message: 'Derivation' }),
            value: derivationLabel(account) ?? '',
          },
        ]
      : []),
    ...(account.hardware
      ? [{ label: t({ id: 'acct.details.path', message: 'Path' }), value: account.hardware.path }]
      : []),
    ...(account.hardware?.deviceId
      ? [
          {
            label: t({ id: 'acct.details.device', message: 'Device' }),
            value: account.hardware.deviceId,
          },
        ]
      : []),
    {
      label: t({ id: 'acct.details.signs', message: 'Signs' }),
      value: account.hasKey
        ? t({ id: 'acct.details.signs.here', message: 'Here, with your password' })
        : account.kind === 'watch'
          ? t({ id: 'acct.details.signs.no', message: 'Never — watch only' })
          : t({ id: 'acct.details.signs.device', message: 'On the device' }),
    },
    {
      label: t({ id: 'acct.details.added', message: 'Added' }),
      ...
      <Column gap="$3">
        <Row gap="$2" alignItems="center">
          ...
          <IconButton
            ...
            label={t({ id: 'token.copyAddress', message: 'Copy address' })}
            active={copied}
            onPress={() => void copy()}
            testID="account-details-copy"
          />
        </Row>
        <Column gap={6}>
          {rows.map((r) => (
