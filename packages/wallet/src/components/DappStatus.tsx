 ...
 */
import { Artwork, Body, Column, Dot, Icon, Key, Pill, Plate, Pressable, Row, Sheet, Signature, paint, shortAddress } from '@boltvault/ui'
...
export type DappState = { readonly kind: 'none' } | { readonly kind: 'off'; readonly origin: string; readonly host: string; readonly favicon: string | null } | { readonly kind: 'on'; readonly origin: string; readonly host: string; readonly favicon: string | null; readonly site: SiteView }

/** Null while the tab and the site list are still loading. */
export function useDappStatus(enabled: boolean): DappState | null {
  const host = useHost()
  const engine = useEngine()
  const [tab, setTab] = useState<{ origin: string; host: string; favicon: string | null } | null | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    let alive = true
    host.currentTab().then((x) => alive && setTab(x), () => alive && setTab(null))
    return () => {
      alive = false
    }
  }, [host, enabled])
  useEffect(() => {
    if (!enabled) return
    let alive = true
    engine.sites.list().then((s) => alive && setSites(s), () => alive && setSites([]))
...
export function DappStrip({ state, onPress, testID = 'home-dapp' }: { state: DappState | null; onPress: () => void; testID?: string }) {
  const none = !state || state.kind === 'none'
  const label = !state ? '' : state.kind === 'none' ? t({ id: 'dapp.none', message: 'No dApp on this tab' }) : state.host
  const status = state?.kind === 'on' ? t({ id: 'dapp.on', message: 'Connected' }) : state?.kind === 'off' ? t({ id: 'dapp.off', message: 'Not connected' }) : ''
  return (
...
export function DappSheet({ open, onClose, state, reducedMotion = false }: { open: boolean; onClose: () => void; state: DappState | null; reducedMotion?: boolean }) {
  const engine = useEngine()
  ...
  const chainName = (id: number): string => chains.find((c) => c.chainId === id)?.name ?? (id === 52014 ? 'Electroneum' : `Chain ${id}`)
  ...
  return (
    <>
      <Sheet open={open} onClose={onClose} title={state.host} reducedMotion={reducedMotion} footer={site ? <Key label={t({ id: 'sites.disconnect', message: 'Disconnect' })} kind="danger" size="compact" disabled={busy} onPress={() => void disconnect()} testID="dapp-disconnect" /> : undefined} testID="dapp-sheet">
        <Column gap="$3">
          {site ? (
            <>
              <Row gap="$2" alignItems="center">
                ...
                <Body size="caption" tone="mute" flexShrink={1}>
                  {t({ id: 'dapp.on.body', message: 'Connected — this site sees the address below and can ask you to sign. It cannot move anything without a signature.' })}
                </Body>
              </Row>
              <Plate role="recessed" gap={4} paddingVertical={8} testID="dapp-details">
                <Row justifyContent="space-between" alignItems="center" minHeight={36}>
                  ...
                  <ChainSelectPill chainId={site.chainId} label={chainName(site.chainId)} size="sm" onPress={() => setChainOpen(true)} testID="dapp-chain" />
                </Row>
              ...
              <Pill label={t({ id: 'dapp.sites', message: 'All connected sites' })} icon={<Icon name="chevronRight" size={14} color={paint.mute} />} size="sm" onPress={() => { onClose(); router.navigate('sites') }} testID="dapp-sites" />
            </>
          ) : (
            <>
              <Body tone="mute" size="caption">
                {t({ id: 'dapp.off.body', message: '{h} is not connected to BoltVault. Connect from the site itself: it will ask, and you choose the account.', values: { h: state.host } })}
              </Body>
              <Pill label={t({ id: 'dapp.sites', message: 'All connected sites' })} icon={<Icon name="chevronRight" size={14} color={paint.mute} />} size="sm" onPress={() => { onClose(); router.navigate('sites') }} testID="dapp-sites" />
            </>
          )}
      ...
      {site ? (
        <ChainSheet
          ...
          onSelect={(id) => {
            if (id !== 'all') void engine.sites.setChain({ origin: site.origin, chainId: id }).catch(() => undefined)
            setChainOpen(false)
          }}
