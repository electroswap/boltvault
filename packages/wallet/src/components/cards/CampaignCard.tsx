...
export function phaseLabel(c: CampaignView, now = Math.floor(Date.now() / 1000)): string {
  const countdown = (): string => {
    ...
    return d > 0
      ? t({ id: 'sky.days', message: '{d}d {h}h', values: { d, h } })
      : t({
          id: 'sky.hours',
          message: '{h}h {m}m',
          values: { h, m: Math.floor((secs % 3600) / 60) },
        })
  }
  switch (c.phase) {
    case 'live': {
      ...
      return left
        ? t({ id: 'sky.live', message: 'Live · ends in {t}', values: { t: left } })
        : t({ id: 'sky.live.now', message: 'Live now' })
    }
    case 'upcoming': {
      const left = countdown()
      return left
        ? t({ id: 'sky.upcoming', message: 'Starts in {t}', values: { t: left } })
        : t({ id: 'sky.upcoming.soon', message: 'Starting soon' })
    }
...
export function phaseTone(
  phase: CampaignView['phase'],
): 'arc' | 'mute' | 'surge' | 'burn' | 'ember' {
  return phase === 'live'
    ? 'arc'
    : phase === 'launched'
      ? 'surge'
...
export function PhasePill({ c }: { c: CampaignView }) {
  return (
    <Pill
      label={
        c.phase === 'live'
          ? t({ id: 'phase.live', message: 'Live' })
          : c.phase === 'upcoming'
            ? t({ id: 'phase.upcoming', message: 'Upcoming' })
            : c.phase === 'awaiting_finalize'
              ? t({ id: 'phase.finalizing', message: 'Finalizing' })
              : c.phase === 'launched'
                ? t({ id: 'phase.launched', message: 'Launched' })
                : c.phase === 'failed'
                  ? t({ id: 'phase.failed', message: 'Failed' })
                  : t({ id: 'phase.cancelled', message: 'Cancelled' })
      }
      tone={phaseTone(c.phase)}
      size="sm"
    />
  )
}
    ...
    <SharedElement id={campaignSharedId(c.chainId, c.pool)}>
      <Plate role="card" gap="$2" onPress={onPress} cursor="pointer" testID={`sky-${c.pool}`}>
        <Row gap="$3" alignItems="center">
          <TokenAvatar
            chainId={c.chainId}
            address={c.token.address ?? c.pool}
            ...
            <IconButton
              ...
              label={
                c.starred
                  ...
                  : t({ id: 'campaign.star.short', message: 'Tell me when it goes live' })
              }
              active={c.starred}
              onPress={onStar}
              testID={`star-campaign-${c.pool}`}
            />
          ) : null}
        </Row>
        <Row gap="$2" alignItems="center">
          ...
          <Body tone="mute" size="caption">
            {t({
              id: 'sky.raised',
              message: '{r} / {t} ETN',
              values: { r: formatRaw(c.raisedWei, 18), t: formatRaw(c.minEtnToLaunchWei, 18) },
            })}
          </Body>
        </Row>
      </Plate>
