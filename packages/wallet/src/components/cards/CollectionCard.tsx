...
export function CollectionCard({ collection, onPress, onStar }: { collection: CollectionView; onPress: () => void; onStar?: () => void }) {
  const line = [
    ...
    collection.floorEtn !== null ? t({ id: 'explore.floor', message: 'Floor {f} ETN', values: { f: formatRaw(String(Math.round(collection.floorEtn * 1e6)), 6) } }) : null,
    collection.volume24hEtn !== null ? t({ id: 'explore.vol24', message: '{v} ETN today', values: { v: Math.round(collection.volume24hEtn) } }) : null,
    collection.percentListed !== null ? t({ id: 'explore.listed', message: '{p}% listed', values: { p: Math.round(collection.percentListed) } }) : null,
    collection.owned > 0 ? t({ id: 'explore.own', message: 'you own {n}', values: { n: collection.owned } }) : null,
  ]
    .filter(Boolean)
    .join(' · ')
  return (
