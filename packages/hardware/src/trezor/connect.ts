...
const Failure = z.object({
  ...
  payload: z.object({ error: z.string(), code: z.string().optional() }),
})
const ok = <T extends z.ZodTypeAny>(payload: T) =>
  z.union([z.object({ success: z.literal(true), payload }), Failure])

export const TrezorAddressResult = ok(
  z.object({
    address: z.string(),
    path: z.array(z.number()).optional(),
    serializedPath: z.string().optional(),
  }),
)
export const TrezorAddressBundleResult = ok(
  z.array(z.object({ address: z.string(), serializedPath: z.string().optional() })),
)
export const TrezorSignatureResult = ok(
  z.object({ v: z.union([z.string(), z.number()]), r: z.string(), s: z.string() }),
)
export const TrezorMessageResult = ok(z.object({ address: z.string(), signature: z.string() }))
export const TrezorFeaturesResult = ok(
  z.object({
    model: z.string().optional(),
    internal_model: z.string().optional(),
    label: z.string().nullable().optional(),
    major_version: z.number().optional(),
    minor_version: z.number().optional(),
    patch_version: z.number().optional(),
    initialized: z.boolean().optional(),
  }),
)

export type TrezorResult<T> =
  { success: true; payload: T } | { success: false; payload: { error: string; code?: string } }
...
export interface TrezorConnectLike {
  ...
  ethereumGetAddress(input: {
    path: string
    showOnTrezor?: boolean
  }): Promise<TrezorResult<{ address: string; serializedPath?: string }>>
  ethereumGetAddressBundle(input: {
    bundle: Array<{ path: string; showOnTrezor: false }>
  }): Promise<TrezorResult<Array<{ address: string; serializedPath?: string }>>>
  ethereumSignTransaction(input: {
    path: string
    transaction: TrezorTransactionInput
  }): Promise<TrezorResult<{ v: string | number; r: string; s: string }>>
  ethereumSignMessage(input: {
    path: string
    message: string
    hex: boolean
  }): Promise<TrezorResult<{ address: string; signature: string }>>
  ethereumSignTypedData(input: {
    path: string
    data: unknown
...
export function trezorErrorMessage(payload: { error: string; code?: string }): string {
  ...
  if (code === 'Failure_ActionCancelled' || /cancel/i.test(payload.error))
    return 'Cancelled on the Trezor.'
  if (code === 'Method_PermissionsNotGranted' || /permission/i.test(payload.error))
    return 'Trezor Connect needs your permission in its window.'
  if (/device disconnected|no device/i.test(payload.error))
    return 'No Trezor is connected. Plug it in and unlock it.'
...
export function yParityFromTrezorV(v: string | number): 0 | 1 {
  const n =
    typeof v === 'number' ? v : v.startsWith('0x') ? Number.parseInt(v, 16) : Number.parseInt(v, 10)
  if (!Number.isFinite(n) || n < 0)
    throw new TrezorError(
      'Failure_DataError',
      'The Trezor returned a signature the wallet cannot read.',
    )
