 ...
 */
import { getTypesForEIP712Domain, hashDomain, hashStruct, hexToBytes, serializeTransaction, toHex, type Hex, type TransactionSerializable, type TypedDataDefinition } from 'viem'
...
export function trezorAccount(input: TrezorAccountInput): LocalAccount {
  ...
  return toAccount({
    ...
    async signMessage({ message }) {
      ...
      const r = unwrap(await connect.ethereumSignMessage({ path, message: toHex(bytes).slice(2), hex: true }))
    ...
    async signTransaction(transaction, options) {
      ...
      const fields = {
        ...
        ...(legacy ? { gasPrice: hex((tx as { gasPrice?: bigint }).gasPrice) } : { maxFeePerGas: hex((tx as { maxFeePerGas?: bigint }).maxFeePerGas), maxPriorityFeePerGas: hex((tx as { maxPriorityFeePerGas?: bigint }).maxPriorityFeePerGas) }),
      }
    ...
    async sign() {
      throw new TrezorError('Method_Unsupported', 'A Trezor will not sign a raw hash. Use a signed message or a transaction instead.')
    },
  })
}
...
export function normaliseSignature(signature: string): Hex {
  ...
  if (s.length !== 130) throw new TrezorError('Failure_DataError', 'The Trezor returned a signature of the wrong length.')
