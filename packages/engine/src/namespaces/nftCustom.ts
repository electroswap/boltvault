...
const metaId = (chainId: number, address: string, tokenId: string): string =>
...
export class CustomCollectionsService {
  ...
  async preview(chainId: number, address: string): Promise<Omit<CustomCollection, 'addedAt'>> {
    ...
    const code = (await this.deps.chains
      .rpc(chainId, 'eth_getCode', [a, 'latest'])
      .catch(() => '0x')) as string
    if (typeof code !== 'string' || code.length <= 2)
      throw new EngineError(
        'invalid_argument',
        'There is no contract at that address on this chain.',
      )
    ...
    const standard: 'ERC721' | 'ERC1155' | null = flag(is1155)
      ? 'ERC1155'
      : flag(is721)
        ? 'ERC721'
        : null
    if (!standard)
      throw new EngineError(
        'invalid_argument',
        'This contract is not an ERC-721 or ERC-1155 collection.',
      )
    return {
      chainId,
      address: a,
  ...
  async remove(chainId: number, address: string): Promise<void> {
    const cur = await this.list(chainId)
    await this.deps.collections.set(
      listId(chainId),
      cur.filter((c) => c.address.toLowerCase() !== address.toLowerCase()),
    )
  }
  ...
  async ownedIds(chainId: number, c: CustomCollection, owner: Hex): Promise<string[]> {
    ...
    if (c.standard === 'ERC721' && c.enumerable) {
      const [bal] = await readMany(this.deps.chains, chainId, [
        { address, abi: ERC721, functionName: 'balanceOf', args: [owner] },
      ])
      const n = bal?.ok && typeof bal.value === 'bigint' ? Number(bal.value) : 0
      if (n === 0) return []
      const calls: ReadCall[] = []
      for (let i = 0; i < Math.min(n, MAX_PIECES); i += 1)
        calls.push({
          address,
          abi: ERC721,
      ...
      return (await readMany(this.deps.chains, chainId, calls)).flatMap((r) =>
        r.ok && typeof r.value === 'bigint' ? [r.value.toString()] : [],
      )
    }
    ...
    const topics =
      c.standard === 'ERC721'
        ? [TRANSFER_TOPIC, null, pad(owner, { size: 32 })]
        : [[TRANSFER_SINGLE_TOPIC, TRANSFER_BATCH_TOPIC], null, null, pad(owner, { size: 32 })]
    for (let start = from; start <= head && candidates.size < MAX_PIECES * 4; start += SCAN_CHUNK) {
      const end = Math.min(head, start + SCAN_CHUNK - 1)
      const logs = (await this.deps.chains
      ...
      for (const log of logs) {
        if (c.standard === 'ERC721') {
          if (log.topics.length === 4 && log.topics[3])
            candidates.add(BigInt(log.topics[3]).toString())
          ...
          for (let i = 0; i < Math.min(count, 64); i += 1)
            candidates.add(BigInt(`0x${words[idsOffset + 1 + i] ?? '0'}`).toString())
        }
      }
    }
    const ids = [...candidates].slice(0, MAX_PIECES * 2)
    if (ids.length === 0) return []
    const checks: ReadCall[] = ids.map((id) =>
      c.standard === 'ERC721'
        ? { address, abi: ERC721, functionName: 'ownerOf', args: [BigInt(id)] }
        : { address, abi: ERC1155, functionName: 'balanceOf', args: [owner, BigInt(id)] },
    )
    ...
    return ids
      .filter((_, i) => {
        ...
        return c.standard === 'ERC721'
          ? typeof r.value === 'string' && r.value.toLowerCase() === owner.toLowerCase()
          : typeof r.value === 'bigint' && r.value > 0n
      })
      .slice(0, MAX_PIECES)
  }
  ...
  async metadata(
    ...
    const address = c.address as Hex
    const [uriRes] = await readMany(this.deps.chains, chainId, [
      c.standard === 'ERC721'
        ? { address, abi: ERC721, functionName: 'tokenURI', args: [BigInt(tokenId)] }
        : { address, abi: ERC1155, functionName: 'uri', args: [BigInt(tokenId)] },
    ])
  ...
  async ownedPieces(
    ...
    collections: Array<CustomCollection & { balance: number }>
    pieces: CustomPiece[]
  }> {
    ...
    for (const c of collections) {
      ...
      for (const id of ids) {
        ...
        out.push({
          address: c.address,
          tokenId: id,
          name: meta?.name ?? `${c.symbol || c.name} #${id}`,
...
export function customCollectionsNamespace(custom: CustomCollectionsService): NamespaceSpec {
  return {
    previewCollection: {
      ...
      handler: (arg) =>
        custom.preview((arg as { chainId: number }).chainId, (arg as { address: string }).address),
    },
    addCollection: {
      input: Chain.extend({ address: z.string() }),
      handler: (arg) =>
        custom.add((arg as { chainId: number }).chainId, (arg as { address: string }).address),
    },
    removeCollection: {
      input: Chain.extend({ address: z.string() }),
      handler: (arg) =>
        custom.remove((arg as { chainId: number }).chainId, (arg as { address: string }).address),
    },
    customCollections: {
      input: Chain,
      handler: (arg) => custom.list((arg as { chainId: number }).chainId),
    },
  }
}
