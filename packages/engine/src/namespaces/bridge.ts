...
export class BridgeService {
  ...
  async origins(): Promise<
    Array<{ chainId: number; enabled: boolean; symbols: Array<'USDC' | 'USDT'> }>
  > {
    const settings = await this.deps.settings.get()
    const out = new Map<number, Set<'USDC' | 'USDT'>>()
    for (const route of WARP_ROUTES)
      for (const e of route.endpoints)
        out.set(e.chainId, new Set([...(out.get(e.chainId) ?? []), route.symbol]))
    return [...out.entries()].map(([chainId, symbols]) => ({
      chainId,
      enabled: chainId === 52014 || settings.enabledChains.includes(chainId),
      symbols: [...symbols],
    }))
  }
  ...
  async routes(fromChainId: number, token?: string): Promise<BridgeRoute[]> {
    ...
    for (const c of corridorsFrom(fromChainId, token)) {
      ...
      out.push({
        symbol: c.symbol,
        fromChainId: c.origin.chainId,
        toChainId: c.destination.chainId,
        token: c.origin.token,
        router: c.origin.router,
  ...
  async quote(input: {
    ...
    const base: BridgeQuote = {
      ...
      symbol: c?.symbol ?? 'USDC',
      token: input.token,
      decimals: c?.origin.decimals ?? 6,
      amountRaw: '0',
      balanceRaw: '0',
      recipient,
      gasQuoteWei: '0',
      txFeeWei: '0',
      feeSymbol: getChain(input.fromChainId)?.nativeCurrency.symbol ?? 'ETH',
      etaMinutes: etaMinutes(input.fromChainId, input.toChainId),
      steps: [],
      recipientCode: { origin: false, destination: null },
      ok: false,
      problems,
    }
    if (!c)
    ...
    const [balance, allowance, gasQuote] = await readMany(d.chains, c.origin.chainId, [
      ...
      {
        address: c.origin.token,
        abi: TOKEN_ROUTER_ABI,
        functionName: 'allowance',
        args: [owner, c.origin.router],
      },
      {
        address: c.origin.router,
        abi: TOKEN_ROUTER_ABI,
        functionName: 'quoteGasPayment',
        args: [dest?.domain ?? c.destination.chainId],
      },
    ])
    ...
    if (gasQuoteWei === null)
      problems.push('The router did not quote the interchain gas; try again in a moment.')
    const native = BigInt(
      String(
        (await d.chains
          .rpc(c.origin.chainId, 'eth_getBalance', [owner, 'latest'])
          .catch(() => '0x0')) ?? '0x0',
      ),
    )
    const gasPrice = BigInt(
      String(
        (await d.chains.rpc(c.origin.chainId, 'eth_gasPrice', []).catch(() => '0x3b9aca00')) ??
          '0x3b9aca00',
      ),
    )
    const txFeeWei = 220_000n * gasPrice
    if ((gasQuoteWei ?? 0n) + txFeeWei > native)
      problems.push(`Not enough ${base.feeSymbol} for the interchain gas plus the network fee.`)
    // A recipient that is a contract here but empty on the destination would receive nothing there (§8.7).
    const [codeOrigin, codeDest] = await Promise.all([
      d.chains.rpc(c.origin.chainId, 'eth_getCode', [recipient, 'latest']).catch(() => '0x'),
      d.chains.rpc(c.destination.chainId, 'eth_getCode', [recipient, 'latest']).catch(() => null),
    ])
    ...
    if (hasCodeOrigin && codeDest !== null && !hasCodeDest)
      problems.push(
        'That address is a contract here but nothing on the destination chain — the tokens would be lost.',
      )
    /*
      A check that could not run is not a check that passed.

      The destination read is `.catch(() => null)`, and `null` — "the
      destination RPC did not answer" — took the same silent path as "the
      recipient has code there". So the one control standing between a
      contract recipient and tokens stranded on the far chain failed open
      whenever the far chain was unreachable, which is exactly when it is
      least safe to guess. It is a problem the person can act on: try again.
    */
    if (hasCodeOrigin && codeDest === null)
      problems.push(
        'Could not reach the destination chain to check that the recipient exists there. Try again in a moment.',
      )
    ...
    return {
      ...
      steps,
      recipientCode: { origin: hasCodeOrigin, destination: codeDest === null ? null : hasCodeDest },
  ...
  async execute(input: {
    ...
    const dest = hyperlaneChain(c.destination.chainId)
    if (!dest)
      throw new EngineError('invalid_argument', 'Destination not in the Hyperlane snapshot.')
      ...
      steps.push({
        ...
        run: () =>
          d.provider.runInternal({
            ...
            origin: 'internal:bridge:approve',
            ...
            tx: {
              ...
              to: c.origin.token,
              value: '0x0',
              data: encodeApproveRouter(c.origin.router, exact ? amount : maxUint256),
            },
            clientRequestId: `bridge:${tag}:approve`,
          }),
      })
    }
    ...
    steps.push({
      ...
      run: async () => {
        const r = await d.provider.runInternal({
          kind: 'send_transaction',
          origin: 'internal:bridge',
          chainId: c.origin.chainId,
          accountId: input.accountId,
          tx: {
            from: account.address,
            to: c.origin.router,
            value: hex(BigInt(q.gasQuoteWei)),
            data: encodeTransferRemote(dest.domain, recipient, amount),
          },
          clientRequestId: `bridge:${tag}:transfer`,
          bridgeRecipient: {
            hasCodeOnOrigin: q.recipientCode.origin,
            hasCodeOnDestination: q.recipientCode.destination,
          },
        })
    ...
    const tick = async (): Promise<void> => {
      ...
      try {
        ...
        if (item.state === 'pending') {
          const receipt = (await d.chains.rpc(item.fromChainId, 'eth_getTransactionReceipt', [
            item.originHash,
          ...
          if (receipt) {
            ...
            if (receipt.status !== '0x1') next = { ...item, state: 'failed' }
              ...
              const destHead = Number(
                (await d.chains.head(item.toChainId).catch(() => null))?.blockNumber ?? 0,
              )
              next = {
                ...item,
                state: messageId ? 'dispatched' : 'failed',
                messageId,
                scanFrom: destHead > 0 ? Math.max(0, destHead - 50) : 0,
              }
            }
          }
          ...
          if (dest && to >= from) {
            const logs = (await d.chains.rpc(item.toChainId, 'eth_getLogs', [
              {
                fromBlock: hex(from),
                toBlock: hex(to),
                address: dest.mailbox,
                topics: [PROCESS_ID_TOPIC, item.messageId],
              },
            ...
            if (logs.length > 0)
              next = {
                ...item,
                state: 'delivered',
                destinationHash: logs[0]?.transactionHash ?? null,
              }
            else next = { ...item, scanFrom: Number(to >= head ? head - 20n : to) }
          }
        }
        if (next.state === 'dispatched' && d.platform.now() - next.startedAt > TIMEOUT_MS)
          next = { ...next, state: 'timeout' }
        if (next !== item) {
          this.items = this.items.map((x) =>
            x.id === id ? { ...next, updatedAt: d.platform.now() } : x,
          )
          await this.persist()
        }
        const after = this.items.find((x) => x.id === id)
        if (after && (after.state === 'pending' || after.state === 'dispatched'))
          this.timers.set(
            id,
  ...
  async resume(): Promise<void> {
    ...
    for (const item of this.items)
      if (item.state === 'pending' || item.state === 'dispatched') this.watch(item.id)
  }
...
export function bridgeNamespace(bridge: BridgeService): NamespaceSpec {
  return {
    ...
    routes: {
      input: z.object({ fromChainId: z.number().int().positive(), token: z.string().optional() }),
      handler: (arg) =>
        bridge.routes(
          (arg as { fromChainId: number }).fromChainId,
    ...
    list: {
      ...
      handler: (arg) => bridge.list((arg as { accountId?: string } | undefined)?.accountId),
    },
    status: {
      input: z.object({ id: z.string() }),
      handler: (arg) => bridge.status((arg as { id: string }).id),
    },
  }
}
