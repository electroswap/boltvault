...
const ERC721 = parseAbi(['function ownerOf(uint256 tokenId) view returns (address)', 'function isApprovedForAll(address owner, address operator) view returns (bool)', 'function setApprovalForAll(address operator, bool approved)', 'function safeTransferFrom(address from, address to, uint256 tokenId)'])
...
export function marketplaceConfig(chainId: 52014 | 5201420): MarketplaceConfig {
  ...
  return { chainId, seaport: A.seaport15 as Hex, conduitKey: A.seaportConduitKey as Hex, conduit: A.seaportConduit as Hex, feeReceiver: A.nftFeeReceiver as Hex, wetn: A.wetn as Hex }
}

function orderView(o: EsOrder, components: OrderComponents | null): OrderView {
  const raw = components ? (o.type === 'LISTING' ? listingPrice(components) : offerPrice(components)) : null
  return { type: o.type, status: o.status, priceEtn: o.priceEtn, priceRaw: raw === null ? null : raw.toString(), orderHash: o.orderHash, maker: o.maker, createdAt: o.createdAt, endAt: o.endAt, actionable: components !== null && !!o.signature }
}

export class NftService {
  constructor(private readonly deps: NftDeps) {}

  private client(chainId: number): { client: ElectroSwapClient; config: MarketplaceConfig; chainId: 52014 | 5201420 } {
    if (!this.deps.electroswap) throw new EngineError('not_implemented', 'The marketplace needs the ElectroSwap API, which is not reachable in this build.')
    if (!isEtn(chainId)) throw new EngineError('invalid_argument', 'The marketplace lives on Electroneum.')
  ...
  private async toView(chainId: 52014 | 5201420, a: EsAsset, me: Hex | null, dividends: bigint | null): Promise<AssetView> {
    const legends = ELECTRONEUM_ADDRESSES[chainId].electricLegends
    const listingComponents = a.listing ? parseProtocolParameters(a.listing.protocolParameters) : null
  ...
  async assets(chainId: number, address: string, opts: { orderBy?: 'PRICE' | 'RARITY'; asc?: boolean; listed?: boolean; traits?: Array<{ name: string; values: string[] }>; query?: string; after?: string; accountId?: string }): Promise<{ assets: AssetView[]; total: number | null; next: string | null }> {
    const { client, chainId: cid } = this.client(chainId)
    const me = opts.accountId ? (await this.account(opts.accountId)).address : null
    const page = await fetchAssets(client, cid, address, { ...(opts.orderBy ? { orderBy: opts.orderBy } : {}), ...(opts.asc !== undefined ? { asc: opts.asc } : {}), ...(opts.listed !== undefined ? { listed: opts.listed } : {}), ...(opts.traits ? { traits: opts.traits } : {}), ...(opts.query ? { query: opts.query } : {}), ...(opts.after ? { after: opts.after } : {}) })
    const assets: AssetView[] = []
  ...
  async asset(chainId: number, address: string, tokenId: string, accountId?: string): Promise<AssetView | null> {
    const { client, chainId: cid } = this.client(chainId)
    const a = await fetchAsset(client, cid, address, tokenId)
    if (!a) return null
    const [ownerRead] = await readMany(this.deps.chains, cid, [{ address: address as Hex, abi: ERC721, functionName: 'ownerOf', args: [BigInt(tokenId)] }])
    ...
    const dividends = same(address, ELECTRONEUM_ADDRESSES[cid].electricLegends) ? await this.deps.legends.claimableFor(cid, BigInt(tokenId)).catch(() => 0n) : null
    ...
    if (view.listing && chainOwner && !same(view.listing.maker, chainOwner)) return { ...view, listing: null }
    return view
  }
  ...
  async offers(accountId: string, chainId: number): Promise<OffersInbox> {
    ...
    const made = (await fetchBids(client, cid, account.address).catch(() => [])).map((b) => ({ address: b.address, tokenId: b.tokenId, name: b.name, imageUrl: b.imageUrl, collectionName: b.collectionName, offer: orderView(b.order, parseProtocolParameters(b.order.protocolParameters)), expiresAt: b.expiresAt }))
    const obligation = await fetchBidObligation(client, cid, account.address).catch(() => 0n)
    const [bal] = await readMany(this.deps.chains, cid, [{ address: config.wetn, abi: ERC20, functionName: 'balanceOf', args: [account.address] }])
    return { received, made, obligationWei: obligation.toString(), wetnBalanceWei: (bal?.ok && typeof bal.value === 'bigint' ? bal.value : 0n).toString() }
  }

  // ---- flows ---------------------------------------------------------------------------------

  private async counter(chainId: 52014 | 5201420, config: MarketplaceConfig, offerer: Hex): Promise<bigint> {
    const [r] = await readMany(this.deps.chains, chainId, [{ address: config.seaport, abi: SEAPORT_ABI, functionName: 'getCounter', args: [offerer] }])
    return r?.ok && typeof r.value === 'bigint' ? r.value : 0n
  }

  private approveCollectionStep(chainId: number, accountId: string, owner: Hex, collection: Hex, conduit: Hex): FlowStepRun {
    return {
      step: 'approve_collection',
      waitReceipt: true,
      run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:approve', chainId, accountId, tx: { from: owner, to: collection, value: '0x0', data: encodeFunctionData({ abi: ERC721, functionName: 'setApprovalForAll', args: [conduit, true] }) }, clientRequestId: `nft:approve:${collection}:${this.deps.platform.now()}` }),
    }
  }

  private async needsCollectionApproval(chainId: 52014 | 5201420, owner: Hex, collection: Hex, conduit: Hex): Promise<boolean> {
    const [r] = await readMany(this.deps.chains, chainId, [{ address: collection, abi: ERC721, functionName: 'isApprovedForAll', args: [owner, conduit] }])
  ...
  async list(input: { accountId: string; chainId: number; address: string; tokenId: string; priceEtn: string; days: number }): Promise<{ flowId: string; requestId: string | null }> {
    const { client, chainId, config } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    if (account.kind === 'watch') throw new EngineError('invalid_argument', 'Watch-only — import a key or pair a device to list.')
    ...
    if (!Number.isFinite(input.days) || input.days < 1 || input.days > 180) throw new EngineError('invalid_argument', 'Choose between 1 and 180 days.')
    const asset = await this.asset(input.chainId, input.address, input.tokenId, input.accountId)
    if (!asset) throw new EngineError('not_found', 'That piece is not in the marketplace index yet.')
    if (!asset.mine) throw new EngineError('invalid_argument', 'You do not own this piece.')
    if (asset.listing && asset.listing.priceRaw && BigInt(asset.listing.priceRaw) < price) throw new EngineError('invalid_argument', 'A new listing cannot raise the price — cancel the current one first.')
    const collection = input.address as Hex
    const steps: FlowStepRun[] = []
    if (await this.needsCollectionApproval(chainId, account.address, collection, config.conduit)) steps.push(this.approveCollectionStep(chainId, input.accountId, account.address, collection, config.conduit))
    const counter = await this.counter(chainId, config, account.address)
    const endTime = BigInt(Math.floor(this.deps.platform.now() / 1000) + Math.round(input.days * 86_400))
    const order = buildListing({ config, seller: account.address, token: collection, tokenId: BigInt(input.tokenId), standard: asset.standard === 'ERC1155' ? 'ERC1155' : 'ERC721', priceWei: price, creatorFee: asset.creatorFee ? { payoutAddress: asset.creatorFee.payoutAddress as Hex, basisPoints: asset.creatorFee.basisPoints } : null, endTime, counter, salt: saltFrom(this.deps.platform.random(32)), now: this.deps.platform.now() })
    let signature: Hex | null = null
    steps.push({
      step: 'sign_order',
      run: async () => {
        const r = await this.deps.provider.runInternal({ kind: 'sign_typed_data', origin: 'internal:nft:list', chainId, accountId: input.accountId, from: account.address, typedData: orderTypedData(config, order), version: 'v4', clientRequestId: `nft:list:${collection}:${input.tokenId}:${this.deps.platform.now()}` })
    ...
    steps.push({
      ...
      run: async () => {
        ...
        const res = await client.postOrder(orderIntakeBody({ type: 'LISTING', config, token: collection, tokenId: BigInt(input.tokenId), order, signature }))
        if (!res.ok) throw new EngineError('internal', res.message ?? 'The marketplace refused the listing.')
        return { requestId: null, result: Promise.resolve(orderHash(order)) }
      },
    })
    const flow = await this.deps.flows.start({ kind: 'nft', accountId: input.accountId, chainId, quote: null, steps })
  ...
  async offer(input: { accountId: string; chainId: number; address: string; tokenId: string; priceEtn: string; days: number }): Promise<{ flowId: string; requestId: string | null }> {
    const { client, chainId, config } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    if (account.kind === 'watch') throw new EngineError('invalid_argument', 'Watch-only — import a key or pair a device to make offers.')
    const price = parseUnits(input.priceEtn.trim() || '0', 18)
    if (price <= 0n) throw new EngineError('invalid_argument', 'Enter an offer above zero.')
    if (!Number.isFinite(input.days) || input.days < 1 || input.days > 180) throw new EngineError('invalid_argument', 'Choose between 1 and 180 days.')
    const asset = await this.asset(input.chainId, input.address, input.tokenId, input.accountId)
    if (!asset || !asset.owner) throw new EngineError('not_found', 'That piece is not in the marketplace index yet.')
    if (asset.mine) throw new EngineError('invalid_argument', 'You already own this piece.')
    const [wetnBal, native] = await Promise.all([
      readMany(this.deps.chains, chainId, [{ address: config.wetn, abi: ERC20, functionName: 'balanceOf', args: [account.address] }, { address: config.wetn, abi: ERC20, functionName: 'allowance', args: [account.address, config.conduit] }]),
      this.deps.chains.rpc(chainId, 'eth_getBalance', [account.address, 'latest']).catch(() => '0x0'),
    ])
    ...
    if (shortfall > BigInt(String(native))) throw new EngineError('invalid_argument', 'Not enough ETN to back this offer alongside your open ones.')
    const steps: FlowStepRun[] = []
    if (shortfall > 0n) steps.push({ step: 'wrap', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:wrap', chainId, accountId: input.accountId, tx: { from: account.address, to: config.wetn, value: hex(shortfall), data: encodeFunctionData({ abi: WETH, functionName: 'deposit' }) }, clientRequestId: `nft:wrap:${this.deps.platform.now()}` }) })
    if (allowance < needed) steps.push({ step: 'approve', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:approve', chainId, accountId: input.accountId, tx: { from: account.address, to: config.wetn, value: '0x0', data: encodeFunctionData({ abi: ERC20, functionName: 'approve', args: [config.conduit, needed] }) }, clientRequestId: `nft:approve-wetn:${this.deps.platform.now()}` }) })
    const counter = await this.counter(chainId, config, account.address)
    const endTime = BigInt(Math.floor(this.deps.platform.now() / 1000) + Math.round(input.days * 86_400))
    const order = buildOffer({ config, bidder: account.address, owner: asset.owner as Hex, token: input.address as Hex, tokenId: BigInt(input.tokenId), priceWei: price, creatorFee: asset.creatorFee ? { payoutAddress: asset.creatorFee.payoutAddress as Hex, basisPoints: asset.creatorFee.basisPoints } : null, endTime, counter, salt: saltFrom(this.deps.platform.random(32)), now: this.deps.platform.now() })
    let signature: Hex | null = null
    steps.push({
      step: 'sign_order',
      run: async () => {
        const r = await this.deps.provider.runInternal({ kind: 'sign_typed_data', origin: 'internal:nft:offer', chainId, accountId: input.accountId, from: account.address, typedData: orderTypedData(config, order), version: 'v4', clientRequestId: `nft:offer:${input.address}:${input.tokenId}:${this.deps.platform.now()}` })
        const settled = r.result.then((sig) => {
          signature = sig as Hex
          return sig
        })
    ...
    steps.push({
      ...
      run: async () => {
        if (!signature) throw new EngineError('internal', 'no signature')
        const res = await client.postOrder(orderIntakeBody({ type: 'BID', config, token: input.address as Hex, tokenId: BigInt(input.tokenId), order, signature }))
        if (!res.ok) throw new EngineError('internal', res.message ?? 'The marketplace refused the offer.')
        return { requestId: null, result: Promise.resolve(orderHash(order)) }
      },
    })
    const flow = await this.deps.flows.start({ kind: 'nft', accountId: input.accountId, chainId, quote: null, steps })
  ...
  private async liveOrder(chainId: 52014 | 5201420, config: MarketplaceConfig, address: string, tokenId: string, which: 'listing' | { orderHash: string }): Promise<{ components: OrderComponents; signature: Hex; view: EsOrder }> {
    ...
    const view = which === 'listing' ? a.listing : (a.bids.find((b) => b.orderHash && same(b.orderHash, which.orderHash)) ?? null)
    if (!view || !view.signature) throw new EngineError('not_found', which === 'listing' ? 'This piece is not listed right now.' : 'That offer is gone.')
  ...
  async buy(input: { accountId: string; chainId: number; address: string; tokenId: string }): Promise<{ flowId: string; requestId: string | null }> {
    const { chainId, config } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    const { components, signature } = await this.liveOrder(chainId, config, input.address, input.tokenId, 'listing')
    const [ownerRead] = await readMany(this.deps.chains, chainId, [{ address: input.address as Hex, abi: ERC721, functionName: 'ownerOf', args: [BigInt(input.tokenId)] }])
    const owner = ownerRead?.ok && typeof ownerRead.value === 'string' ? ownerRead.value : null
    if (!owner || !same(owner, components.offerer)) throw new EngineError('invalid_argument', 'The seller no longer owns this piece; the listing is stale.')
    if (same(owner, account.address)) throw new EngineError('invalid_argument', 'You already own this piece.')
    ...
    const flow = await this.deps.flows.start({
      ...
      chainId,
      quote: null,
      steps: [{ step: 'buy', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:buy', chainId, accountId: input.accountId, tx: { from: account.address, to: config.seaport, value: hex(value), data }, clientRequestId: `nft:buy:${input.address}:${input.tokenId}:${this.deps.platform.now()}` }) }],
    })
    return { flowId: flow.id, requestId: flow.steps[0]?.requestId ?? null }
  }

  /** Accept an offer on my piece: approve the conduit if needed, then fulfil the bid (§8.10 Accept). */
  async accept(input: { accountId: string; chainId: number; address: string; tokenId: string; orderHash: string }): Promise<{ flowId: string; requestId: string | null }> {
    const { chainId, config } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    const { components, signature } = await this.liveOrder(chainId, config, input.address, input.tokenId, { orderHash: input.orderHash })
    const piece = components.consideration.find((c) => c.itemType === ItemType.ERC721 || c.itemType === ItemType.ERC1155)
    if (!piece || !same(piece.token, input.address) || piece.identifierOrCriteria !== BigInt(input.tokenId)) throw new EngineError('invalid_argument', 'That offer is for a different piece.')
    const steps: FlowStepRun[] = []
    if (await this.needsCollectionApproval(chainId, account.address, input.address as Hex, config.conduit)) steps.push(this.approveCollectionStep(chainId, input.accountId, account.address, input.address as Hex, config.conduit))
    const { data } = encodeFulfillOrder(toParameters(components), signature, config.conduitKey)
    steps.push({ step: 'accept', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:accept', chainId, accountId: input.accountId, tx: { from: account.address, to: config.seaport, value: '0x0', data }, clientRequestId: `nft:accept:${input.orderHash}:${this.deps.platform.now()}` }) })
    const flow = await this.deps.flows.start({ kind: 'nft', accountId: input.accountId, chainId, quote: null, steps })
    return { flowId: flow.id, requestId: flow.steps[0]?.requestId ?? null }
  }

  /** Cancel my listing or my offer on chain (§8.10 Cancel). */
  async cancel(input: { accountId: string; chainId: number; address: string; tokenId: string; orderHash: string }): Promise<{ flowId: string; requestId: string | null }> {
    const { chainId, config } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    const listing = await this.liveOrder(chainId, config, input.address, input.tokenId, 'listing').catch(() => null)
    const target = listing && listing.view.orderHash && same(listing.view.orderHash, input.orderHash) ? listing : await this.liveOrder(chainId, config, input.address, input.tokenId, { orderHash: input.orderHash })
    if (!same(target.components.offerer, account.address)) throw new EngineError('invalid_argument', 'Only the maker can cancel an order.')
    const flow = await this.deps.flows.start({
      kind: 'nft',
      accountId: input.accountId,
      chainId,
      quote: null,
      steps: [{ step: 'cancel_order', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:cancel', chainId, accountId: input.accountId, tx: { from: account.address, to: config.seaport, value: '0x0', data: encodeCancel([target.components]) }, clientRequestId: `nft:cancel:${input.orderHash}:${this.deps.platform.now()}` }) }],
    })
    return { flowId: flow.id, requestId: flow.steps[0]?.requestId ?? null }
  }

  /** Transfer a piece (`safeTransferFrom`) — the recipient goes through the same poison checks as a send. */
  async transfer(input: { accountId: string; chainId: number; address: string; tokenId: string; to: string }): Promise<{ flowId: string; requestId: string | null }> {
    const { chainId } = this.client(input.chainId)
    const account = await this.account(input.accountId)
    if (!/^0x[0-9a-fA-F]{40}$/.test(input.to)) throw new EngineError('invalid_argument', 'Enter a full address.')
    const flow = await this.deps.flows.start({
      kind: 'nft',
      accountId: input.accountId,
      chainId,
      quote: null,
      steps: [{ step: 'transfer', waitReceipt: true, run: () => this.deps.provider.runInternal({ kind: 'send_transaction', origin: 'internal:nft:transfer', chainId, accountId: input.accountId, tx: { from: account.address, to: input.address as Hex, value: '0x0', data: encodeFunctionData({ abi: ERC721, functionName: 'safeTransferFrom', args: [account.address, input.to as Hex, BigInt(input.tokenId)] }) }, clientRequestId: `nft:transfer:${input.address}:${input.tokenId}:${this.deps.platform.now()}` }) }],
    })
    return { flowId: flow.id, requestId: flow.steps[0]?.requestId ?? null }
  }
  ...
  async collectionApproved(accountId: string, chainId: number, address: string): Promise<boolean> {
    ...
    const account = await this.account(accountId)
    return !(await this.needsCollectionApproval(cid, account.address, address as Hex, config.conduit))
  }
  ...
  async ownerOf(chainId: number, address: string, tokenId: string): Promise<string | null> {
    ...
    const [r] = await readMany(this.deps.chains, chainId, [{ address: address as Hex, abi: LEGENDS_ABI, functionName: 'ownerOf', args: [BigInt(tokenId)] }])
...
const OrderInput = AccountPiece.extend({ priceEtn: z.string().max(40), days: z.number().min(1).max(180) })
...
export function nftNamespace(nft: NftService): NamespaceSpec {
  return {
    ...
    assets: {
      ...
      handler: (arg) => {
        const a = arg as { chainId: number; address: string; orderBy?: 'PRICE' | 'RARITY'; asc?: boolean; listed?: boolean; traits?: Array<{ name: string; values: string[] }>; query?: string; after?: string; accountId?: string }
        return nft.assets(a.chainId, a.address, a)
      },
    },
    asset: { input: Piece.extend({ accountId: AccountIdSchema.optional() }), handler: (arg) => nft.asset((arg as { chainId: number }).chainId, (arg as { address: string }).address, (arg as { tokenId: string }).tokenId, (arg as { accountId?: string }).accountId) },
    activity: { input: Chain.extend({ address: z.string(), tokenId: z.string().optional() }), handler: (arg) => nft.activity((arg as { chainId: number }).chainId, (arg as { address: string }).address, (arg as { tokenId?: string }).tokenId) },
    offers: { input: Chain.extend({ accountId: AccountIdSchema }), handler: (arg) => nft.offers((arg as { accountId: string }).accountId, (arg as { chainId: number }).chainId) },
    list: { input: OrderInput, handler: (arg) => nft.list(arg as { accountId: string; chainId: number; address: string; tokenId: string; priceEtn: string; days: number }) },
    offer: { input: OrderInput, handler: (arg) => nft.offer(arg as { accountId: string; chainId: number; address: string; tokenId: string; priceEtn: string; days: number }) },
    buy: { input: AccountPiece, handler: (arg) => nft.buy(arg as { accountId: string; chainId: number; address: string; tokenId: string }) },
