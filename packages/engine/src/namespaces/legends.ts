...
const flag = (r: { ok: boolean; value?: unknown } | undefined): boolean =>
  r?.ok === true && r.value === true
...
export class LegendsService {
  ...
  async status(accountId: string, chainId: number): Promise<LegendsStatus | null> {
    ...
    if (balance > 0) {
      ...
      for (let i = 0; i < Math.min(balance, 200); i++)
        calls.push({
          address: collection,
          abi: LEGENDS_ABI,
      ...
      for (const r of await readMany(d.chains, chainId, calls))
        if (r.ok && typeof r.value === 'bigint') ids.push(r.value)
    }
    const info = ids.length
      ? await readMany(
          ...
          ids.map((id) => ({
            address: distributor,
            abi: DIVIDENDS_ABI,
            functionName: 'tokenDividendInfo',
            args: [id],
          })),
        )
      : []
    ...
    ids.forEach((id, i) => {
      const r = info[i]
      const isRegistered =
        r?.ok && Array.isArray(r.value) ? Boolean((r.value as [bigint, bigint, boolean])[2]) : false
      ;(isRegistered ? registered : unregistered).push(id)
    })
    const unique = claimableIds(registered)
    const claimable = unique.length
      ? big(
          (
            await readMany(d.chains, chainId, [
              {
                address: distributor,
                abi: DIVIDENDS_ABI,
    ...
    return {
      ...
      mint:
        head[4]?.ok === true
          ? {
              mintable,
              priceWei: big(head[5]).toString(),
              mintableCount: Number(big(head[6])),
              totalSupply: Number(big(head[7])),
            }
          : null,
  ...
  async mintInfo(
    ...
    return {
      mintable: flag(r[0]),
      priceWei: big(r[1]).toString(),
      mintableCount: Number(big(r[2])),
      totalSupply: Number(big(r[3])),
    }
  }
      ...
      steps: [
        {
          ...
          run: () =>
            this.deps.provider.runInternal({
              ...
              origin: 'internal:legends:register',
              chainId,
              accountId,
              tx: {
                from: account.address,
                to: st.distributor as Hex,
                value: '0x0',
                data: encodeRegister(ids),
              },
              clientRequestId: `legends:register:${this.deps.platform.now()}`,
            }),
        },
      ],
    })
  ...
  async claim(
    ...
    if (BigInt(st.claimableWei) === 0n)
      throw new EngineError('invalid_argument', 'Nothing to claim yet.')
    const account = await this.account(accountId)
          ...
          run: async () => {
            const r = await this.deps.provider.runInternal({
              ...
              tx: {
                ...
                to: st.distributor as Hex,
                value: '0x0',
                data: encodeClaimDividends(ids),
              },
              clientRequestId: `legends:claim:${this.deps.platform.now()}`,
            })
  ...
  async mint(
    ...
    this.assertEnabled()
    if (!isEtn(chainId))
      throw new EngineError('invalid_argument', 'Electric Legends live on Electroneum.')
    if (!Number.isInteger(count) || count < 1 || count > 20)
      throw new EngineError('invalid_argument', 'Mint between 1 and 20 at a time.')
    const A = ELECTRONEUM_ADDRESSES[chainId]
    const account = await this.account(accountId)
    const [price, mintable, allowed] = await readMany(this.deps.chains, chainId, [
      {
        address: A.nftMinter as Hex,
        abi: MINTER_ABI,
        functionName: 'mintPrice',
        args: [(collection ?? A.electricLegends) as Hex],
      },
      {
        address: (collection ?? A.electricLegends) as Hex,
        abi: LEGENDS_ABI,
      ...
      },
      {
        ...
        abi: MINTER_ABI,
        functionName: 'mintableCount',
        args: [(collection ?? A.electricLegends) as Hex, account.address],
      },
    ])
    if (!flag(mintable)) throw new EngineError('invalid_argument', 'Minting is closed right now.')
    if (Number(big(allowed)) < count)
      throw new EngineError('invalid_argument', `You can mint ${Number(big(allowed))} more.`)
      ...
      steps: [
        {
          ...
          run: () =>
            this.deps.provider.runInternal({
              kind: 'send_transaction',
              origin: 'internal:nft:mint',
              chainId,
              accountId,
              tx: {
                from: account.address,
                to: A.nftMinter as Hex,
                value: hex(value),
                data: encodeMint((collection ?? A.electricLegends) as Hex, BigInt(count)),
              },
...
export function legendsNamespace(legends: LegendsService): NamespaceSpec {
  return {
    ...
    mint: {
      input: AccountChain.extend({ count: z.number().int().min(1).max(20) }),
      handler: (arg) =>
        legends.mint(
          (arg as { accountId: string }).accountId,
          (arg as { chainId: number }).chainId,
          (arg as { count: number }).count,
        ),
    },
  }
}
