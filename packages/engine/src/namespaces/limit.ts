...
export class LimitService {
  ...
  private manager(chainId: number): Hex | null {
    return isEtn(chainId)
      ? ((ELECTRONEUM_ADDRESSES[chainId].limitOrders as Hex | null) ?? null)
      : null
  }
  ...
  async quote(input: LimitInput): Promise<LimitQuote> {
    ...
    const d = this.deps
    if (!d.enabled)
      throw new EngineError('not_implemented', 'Limit orders are not enabled in this build.')
    const { chainId } = input
    const [inView, outView] = await Promise.all([
      d.tokens.get(chainId, input.tokenIn),
      d.tokens.get(chainId, input.tokenOut),
    ])
    const manager = this.manager(chainId)
    if (!isEtn(chainId) || !manager)
      return this.skeleton(input, inView, outView, ['Limit orders live on Electroneum mainnet.'])
    ...
    if (account.kind === 'watch')
      problems.push('Watch-only — import a key or pair a device to place orders.')
    const status = await d.vault.status()
    if (!status.backupComplete && status.seeds.length > 0 && account.kind === 'hd')
      problems.push('Back up your recovery phrase before you place orders.')
    ...
    for (const side of [inView, outView]) {
      if (await this.isBlocked(input.chainId, side.address))
        problems.push(
          `${side.symbol} is marked unsafe by ElectroSwap. BoltVault will not trade it.`,
        )
    }
    ...
    const p2 =
      state[2]?.ok && Array.isArray(state[2].value)
        ? (state[2].value as [bigint, number, number])
        : null
    const nowS = Math.floor(d.platform.now() / 1000)
    const base: LimitQuote = {
      ...this.skeleton(input, inView, outView, problems),
    ...
    const market = await bestRoute(
      tokenIn,
      tokenOut,
      amountIn,
      quoteAddresses(chainId),
      read,
    ).catch(() => null)
    const marketRate = market
      ? rateOf(amountIn, market.best.amountOut, inView.decimals, outView.decimals)
      : null
    ...
    if ((nativeIn && steps.includes('wrap') ? amountIn - wetnBalance : 0n) + feeWei > nativeBalance)
  ...
  async place(input: LimitInput): Promise<{ flowId: string; requestId: string | null }> {
    ...
    const manager = this.manager(chainId)
    if (!isEtn(chainId) || !manager)
      throw new EngineError('invalid_argument', 'Limit orders live on Electroneum mainnet.')
      ...
      steps.push({
        ...
        run: async () => {
          ...
          const [r] = await readMany(d.chains, chainId, [
            { address: wetn, abi: ERC20_ABI, functionName: 'balanceOf', args: [owner] },
          ])
          ...
          return d.provider.runInternal({
            ...
            clientRequestId: `limit:${tag}:wrap`,
          })
        },
      })
    }
        ...
        run: () =>
          d.provider.runInternal({
            ...
            tx: {
              ...
              to: tokenIn,
              value: '0x0',
              data: encodeApprovePermit2(permit2, settings.exactApprovals ? amountIn : maxUint256)
                .data,
            },
            clientRequestId: `limit:${tag}:approve`,
          }),
      })
    }
      ...
      steps.push({
        ...
        run: async () => {
          ...
          const { requestId, result } = await d.provider.runInternal({
            ...
            origin: 'internal:limit:permit',
            chainId,
            accountId: input.accountId,
            from: owner,
            typedData: typed,
            version: 'v4',
            clientRequestId: `limit:${tag}:permit`,
          })
    ...
    steps.push({
      ...
      run: () => {
        const order = {
          ...
          tokenOut,
          unwrapOutput,
          amountInExact: amountIn,
          amountOutMin: minOut,
          recipient: owner,
          durationSeconds: BigInt(input.durationSeconds),
        }
        const data = permit
          ? encodeSubmitOrderWithPermit({ ...order, permit })
          : encodeSubmitOrder(order)
        return d.provider.runInternal({
          kind: 'send_transaction',
          origin: 'internal:limit',
          chainId,
          accountId: input.accountId,
          tx: { from: owner, to: manager, value: '0x0', data },
          clientRequestId: `limit:${tag}:submit`,
        })
      },
    })
          ...
          run: () =>
            d.provider.runInternal({
              ...
              tx: {
                ...
                to: manager,
                value: '0x0',
                data: encodeCloseOrder(BigInt(input.orderId)),
              },
              clientRequestId: `limit:cancel:${input.orderId}:${d.platform.now()}`,
            }),
        },
      ],
    })
  ...
  async list(input: { accountId: string; chainId: number }): Promise<LimitOrderView[]> {
    ...
    const orders = await openOrders(
      ...
      readerFor(d.chains, input.chainId),
      Math.floor(d.platform.now() / 1000),
    )
    ...
    const meta = (address: Hex): { symbol: string; decimals: number } => {
      ...
      return t
        ...
        : { symbol: `${address.slice(0, 6)}…${address.slice(-4)}`, decimals: 18 }
    }
...
export function limitNamespace(limit: LimitService): NamespaceSpec {
  return {
    ...
    cancel: {
      input: AccountChain.extend({ orderId: z.string().regex(/^\d+$/) }),
      handler: (arg) =>
        limit.cancel(arg as { accountId: string; chainId: number; orderId: string }),
    },
    list: {
      input: AccountChain,
      handler: (arg) => limit.list(arg as { accountId: string; chainId: number }),
    },
  }
}
