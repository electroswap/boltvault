...
const bump = (hex: string): Hex =>
  `0x${((BigInt(hex) * BUMP_NUMERATOR) / BUMP_DENOMINATOR + 1n).toString(16)}`
...
export class TxService {
  ...
  private async pendingRow(id: string): Promise<ActivityEntry> {
    ...
    if (row.nonce === null)
      throw new EngineError('invalid_argument', 'That transaction has no nonce to replace.')
    if (isElectroneumChainId(row.chainId)) {
      // Honest refusal rather than a button that cannot work (§7.10).
      throw new EngineError(
        'invalid_argument',
        'Electroneum settles in about five seconds, so there is nothing to speed up or cancel.',
      )
    }
    return row
  }
  ...
  private async bumpedFees(
    chainId: number,
  ): Promise<{ maxFeePerGas?: Hex; maxPriorityFeePerGas?: Hex; gasPrice?: Hex }> {
    ...
    if (block?.baseFeePerGas) {
      const tip = String(
        (await rpc('eth_maxPriorityFeePerGas', []).catch(() => '0x3b9aca00')) ?? '0x3b9aca00',
      )
      const base = BigInt(block.baseFeePerGas)
      const bumpedTip = bump(tip)
      return {
        maxPriorityFeePerGas: bumpedTip,
        maxFeePerGas: `0x${(base * 2n + BigInt(bumpedTip)).toString(16)}` as Hex,
      }
    }
  ...
  async speedUp(input: { id: string }): Promise<{ requestId: string }> {
    ...
    if (!row.data && !row.to)
      throw new EngineError(
        'invalid_argument',
        'There is not enough of that transaction left to re-send.',
      )
  ...
  async replaceable(input: { id: string }): Promise<{ can: boolean; why: string | null }> {
    try {
      ...
      return {
        can: false,
        why: err instanceof EngineError ? err.message : 'That transaction cannot be replaced.',
      }
    }
  }
}
