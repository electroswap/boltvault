...
export type ApprovalIntent =
  ...
  | { readonly kind: 'send_transaction'; readonly origin: string; readonly chainId: number; readonly accountId: string; readonly tx: TxParams; readonly clientRequestId: string; /** internal:swap only — the fee the encoder wrote, checked by the firewall (T10). */ readonly expectedFee?: { readonly sink: Hex; readonly bips: number } | null; /** internal:bridge only — whether the recipient is a contract here and on the destination (§8.7). */ readonly bridgeRecipient?: { readonly hasCodeOnOrigin: boolean; readonly hasCodeOnDestination: boolean | null } | null }
