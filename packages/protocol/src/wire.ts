...
export type InpageMessage =
  | { readonly target: typeof CONTENT_TARGET; readonly channel: string; readonly kind: 'response'; readonly id: number; readonly result?: unknown; readonly error?: RpcErrorShape }
  | { readonly target: typeof CONTENT_TARGET; readonly channel: string; readonly kind: 'event'; readonly event: string; readonly payload: unknown }

/** bridge ⇄ engine over the Port */
export type ProviderPortMessage =
  | { readonly kind: 'request'; readonly id: number; readonly method: string; readonly params?: unknown; /** The bridge's per-page-load nonce, so a re-sent request re-attaches to its approval. */ readonly session?: string }
  | { readonly kind: 'response'; readonly id: number; readonly result?: unknown; readonly error?: RpcErrorShape }
...
export function isInpageRequest(x: unknown, channel: string): x is InpageRequest {
  return isObj(x) && x['target'] === INPAGE_TARGET && x['channel'] === channel && typeof x['id'] === 'number' && typeof x['method'] === 'string' && x['method'].length > 0 && x['method'].length < 128
}
