...
const unreadable: TrezorResult<never> = {
  success: false,
  payload: {
    error: 'Trezor Connect answered with something the wallet cannot read.',
    code: 'Failure_DataError',
  },
}

function parsed<T>(
  schema: { safeParse(v: unknown): { success: true; data: TrezorResult<T> } | { success: false } },
  value: unknown,
): TrezorResult<T> {
  ...
  return {
    async init(input) {
      ...
      await c.init({
        ...
        connectSrc: input.connectSrc ?? TREZOR_CONNECT_SRC,
        lazyLoad: true,
    ...
    async ethereumGetAddress(input) {
      return parsed(
        ...
        await (
          await load()
        ).ethereumGetAddress({ path: input.path, showOnTrezor: input.showOnTrezor ?? false }),
      )
    },
    async ethereumGetAddressBundle(input) {
      return parsed(
        TrezorAddressBundleResult,
        await (await load()).ethereumGetAddress({ bundle: input.bundle }),
      )
    },
    async ethereumSignTransaction(input) {
      return parsed(
        ...
        await (
          await load()
        ).ethereumSignTransaction({ path: input.path, transaction: { ...input.transaction } }),
      )
    },
    async ethereumSignMessage(input) {
      return parsed(
        ...
        await (
          await load()
        ).ethereumSignMessage({ path: input.path, message: input.message, hex: input.hex }),
      )
    },
    async ethereumSignTypedData(input) {
      const c = await load()
      const r =
        input.domain_separator_hash && input.message_hash
          ? await c.ethereumSignTypedData({
              path: input.path,
              data: input.data,
              metamask_v4_compat: input.metamask_v4_compat,
              domain_separator_hash: input.domain_separator_hash,
