/**
 * TokenAvatar (T4.3) — the logo pipeline as a React component.
 *
 * `pickLogo` is the pure, testable core: given the ordered `logoCandidates`
 * list it walks them with a 2s timeout (HEAD-ish probe via Image onload) and
 * resolves to the first that loads, or `null` (→ the identicon last frame).
 * The component renders an `<img>` that starts on the identicon data-URI and
 * swaps in the winner when one resolves — a bus bar is *never* a blank disc.
 */
 ...
 * `fetchLogo` is injectable for tests.
 */
...
/** Real loader: an off-DOM Image with a 2s timeout. `true` on load, else false. */
...
export function TokenAvatar(props: TokenAvatarProps): ReactNode {
  ...
  const identicon = useMemo(
    () => identiconDataUri(props.address, size),
    [props.address, size],
  )
