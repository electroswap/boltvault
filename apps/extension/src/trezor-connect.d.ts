...
declare module '@trezor/connect-webextension' {
  ...
  export interface TrezorConnectApi {
    ...
    ethereumGetAddress(params: { path: string; showOnTrezor?: boolean } | { bundle: ReadonlyArray<{ path: string; showOnTrezor: boolean }> }): Promise<unknown>
    ethereumSignTransaction(params: { path: string; transaction: Record<string, unknown> }): Promise<unknown>
    ethereumSignMessage(params: { path: string; message: string; hex: boolean }): Promise<unknown>
    ethereumSignTypedData(params: { path: string; data: unknown; metamask_v4_compat: boolean; domain_separator_hash?: string; message_hash?: string }): Promise<unknown>
    dispose(): void
  }
