...
export function scrub(text: string): string {
  return text
    ...
    .replace(/(\?|#)[^\s)]+/g, '$1…')
    .replace(/\b(\w+\s){11}\w+\b/g, (m) => (m.split(/\s+/).every((w) => /^[a-z]{3,8}$/.test(w)) ? '[words]' : m))
}

export function toReport(err: unknown, body: CrashReport['body'], version: string): CrashReport {
  const e = err instanceof Error ? err : new Error(String(err))
  return { message: scrub(e.message).slice(0, 500), stack: e.stack ? scrub(e.stack).slice(0, 4_000) : null, version, body, at: Date.now() }
}

export function installCrashReporter(input: { body: CrashReport['body']; version: string; enabled: () => Promise<boolean>; fetchImpl?: typeof fetch; endpoint?: string }): () => void {
  const send = async (err: unknown): Promise<void> => {
    try {
      if (!(await input.enabled())) return
      const report = toReport(err, input.body, input.version)
      await (input.fetchImpl ?? fetch)(input.endpoint ?? CRASH_ENDPOINT, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(report), keepalive: true })
    } catch {
